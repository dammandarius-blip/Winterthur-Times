...
window.adminReplySupportMessage = async function(chatId) {
    ...
    try {
        ...
        await refreshAdminSupportChatsFromWorker(ADMIN_MASTER_PASSWORD, false);
    } catch (err) {
...
async function refreshAdminSupportChatsFromWorker(password, shouldRender = true) {
    const state = captureAdminSupportScrollState ? captureAdminSupportScrollState() : null;
    ...
    if (nextSnapshot !== adminSupportLastSnapshot) {
        ...
        // WICHTIG:
        // Kein window.saveState() hier. Der Worker ist Quelle für Support-Chats.
        // saveState triggert Firebase/Render und verursacht Sprünge/Eingabelöschung.

        const input = document.getElementById('adminSupportInput');
        const chatContainer = document.getElementById('adminChatContainer');
        const userIsWriting = input && document.activeElement === input && input.value.trim() !== '';
        const userIsReading = chatContainer && (chatContainer.scrollHeight - chatContainer.scrollTop - chatContainer.clientHeight > 120);

        if (shouldRender && adminTab === "support") {
            if (userIsWriting || userIsReading) {
                // Nicht neu rendern, solange Admin schreibt oder hochgescrollt ist.
                window.__wtPendingSupportRender = true;
                restoreAdminSupportScrollState(state);
            } else {
                renderApp();
                restoreAdminSupportScrollState(state);
            }
...
function startAdminSupportAutoRefresh(password) {
    ...
    adminSupportAutoRefreshTimer = setInterval(() => {
        if (!(isSuperAdmin && adminTab === "support")) return;
        ...
        // Während Tippen oder Hochscrollen nicht refresh-rendern.
        refreshAdminSupportChatsFromWorker(password, !(userIsWriting || userIsReading))
            .catch(err => console.error("Admin Auto-Refresh Fehler:", err));
    }, 5000);
}
...
/* =========================================================
   Admin Support-Chat Stabilitätsfix
   - Eingabetext bleibt erhalten
   - Scrollposition bleibt erhalten
   - Refresh springt nicht mehr nach oben
   ========================================================= */
(function wtAdminSupportChatStabilityFix() {
    if (window.__wtAdminSupportChatStabilityFix) return;
    window.__wtAdminSupportChatStabilityFix = true;
    ...
    function isSupportView() {
        try { return adminTab === "support"; } catch (_) { return false; }
    }

    function getContainer() {
        return document.getElementById("adminChatContainer");
    }

    function getInput() {
        return document.getElementById("adminSupportInput");
    }
    ...
    function captureSupportState() {
        ...
        try { state.chatId = String(adminSelectedChatId || ""); } catch (_) {}

        if (input) {
            state.inputValue = input.value || "";
            state.inputFocused = document.activeElement === input;
        ...
        if (container) {
            ...
            state.wasNearBottom = container.scrollHeight - container.scrollTop - container.clientHeight < 80;
        }
    }

    function restoreSupportState() {
        if (!isSupportView()) return;

        const input = getInput();
        const container = getContainer();

        if (input && state.chatId === String(adminSelectedChatId || "")) {
            if (state.inputValue && input.value !== state.inputValue) input.value = state.inputValue;
        ...
        if (container && state.chatId === String(adminSelectedChatId || "")) {
            if (state.wasNearBottom) {
                container.scrollTop = container.scrollHeight;
            } else if (typeof state.scrollTop === "number") {
                const diff = container.scrollHeight - (state.scrollHeight || container.scrollHeight);
                container.scrollTop = Math.max(0, state.scrollTop + Math.max(0, diff));
            }
        }
    }

    // renderApp wrappen, damit jeder automatische Refresh Zustand erhält.
    function patchRenderApp() {
        if (typeof window.renderApp !== "function" || window.renderApp.__wtSupportStablePatched) return;

        const oldRender = window.renderApp;
        const patched = function(...args) {
            captureSupportState();
            const result = oldRender.apply(this, args);
            setTimeout(restoreSupportState, 0);
            setTimeout(restoreSupportState, 80);
            return result;
        };

        patched.__wtSupportStablePatched = true;
        window.renderApp = patched;
    }

    // Worker-Refresh ebenfalls stabilisieren.
    function patchRefreshFunction() {
        if (typeof window.refreshAdminSupportChatsFromWorker !== "function" || window.refreshAdminSupportChatsFromWorker.__wtSupportStablePatched) return;

        const oldRefresh = window.refreshAdminSupportChatsFromWorker;
        const patched = async function(...args) {
            captureSupportState();
        ...
        patched.__wtSupportStablePatched = true;
        window.refreshAdminSupportChatsFromWorker = patched;
    }

    document.addEventListener("input", event => {
        if (event.target && event.target.id === "adminSupportInput") captureSupportState();
    }, true);

    document.addEventListener("scroll", event => {
        if (event.target && event.target.id === "adminChatContainer") captureSupportState();
    }, true);

    setInterval(() => {
        patchRenderApp();
        patchRefreshFunction();
        if (isSupportView()) restoreSupportState();
    }, 500);
...
/* =========================================================
   Admin Ressorts immer aktuell halten
   ========================================================= */
(function wtAdminCategoriesLiveSyncFix() {
    if (window.__wtAdminCategoriesLiveSyncFix) return;
    window.__wtAdminCategoriesLiveSyncFix = true;
    ...
    function applyCats(cats) {
        ...
        const cleaned = Array.from(new Set(cats.map(c => String(c || "").trim()).filter(Boolean)));
        try { categories = cleaned; } catch (_) {}
        window.categories = cleaned;
        try { localStorage.setItem("wt_categories_backup", JSON.stringify(cleaned)); } catch (_) {}

        document.querySelectorAll("select").forEach(select => {
            const id = String(select.id || "").toLowerCase();
            const text = String(select.closest("label, div, p, section")?.textContent || "").toLowerCase();
            if (!id.includes("category") && !id.includes("ressort") && !text.includes("ressort") && !text.includes("kategorie")) return;

            const current = select.value;
            const oldVals = Array.from(select.options || []).map(o => o.value);
            if (!oldVals.some(v => cleaned.includes(v)) && !id.includes("category") && !text.includes("ressort")) return;

            select.innerHTML = cleaned.map(cat => `<option value="${cat.replace(/"/g, "&quot;")}">${cat}</option>`).join("");
            if (current && cleaned.includes(current)) select.value = current;
        });
    }

    window.addEventListener("wtCategoriesUpdated", event => {
        applyCats(event.detail && event.detail.categories ? event.detail.categories : getCats());
    });

    setInterval(() => applyCats(getCats()), 1500);
})();

/* =========================================================
   Admin Benutzer-Login Persistenzschutz
   ========================================================= */
(function wtAdminUserLoginNeverDeleteGuard() {
    if (window.__wtAdminUserLoginNeverDeleteGuard) return;
    window.__wtAdminUserLoginNeverDeleteGuard = true;

    function getUsers() {
        try { if (typeof registeredUsers !== "undefined" && Array.isArray(registeredUsers)) return registeredUsers; } catch (_) {}
        if (Array.isArray(window.registeredUsers)) return window.registeredUsers;
        return [];
    }
    ...
    function backup() {
        ...
        try { localStorage.setItem("wt_registered_users_never_delete_backup", JSON.stringify(users)); } catch (_) {}
    }
    ...
    function restoreIfEmpty() {
        ...
        try {
            const backupUsers = JSON.parse(localStorage.getItem("wt_registered_users_never_delete_backup") || "[]");
            if (Array.isArray(backupUsers) && backupUsers.length) {
                try { registeredUsers = backupUsers; } catch (_) {}
                window.registeredUsers = backupUsers;
                if (typeof renderApp === "function") renderApp();
                return true;
            }
        } catch (_) {}
