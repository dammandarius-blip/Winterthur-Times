...
/* Benutzer-Panel lokales Profilfoto */
(function wtAdminUserProfilePhoto() {
    if (window.__wtAdminUserProfilePhoto) return;
    window.__wtAdminUserProfilePhoto = true;

    function fileToUrl(file) {
        return new Promise((resolve, reject) => {
            if (!file) return reject(new Error("Keine Datei."));
            if (!file.type.startsWith("image/")) return reject(new Error("Bitte Bild auswählen."));
            if (file.size > 2500000) return reject(new Error("Bild unter 2.5 MB verwenden."));
    ...
    function enhance() {
        if (!/Benutzer|Users|Nutzer/i.test(document.body.innerText || "")) return;
        const list = users();
        Array.from(document.querySelectorAll("tr, .bg-white, .border, .rounded")).forEach(row => {
            if (row.dataset.wtPhotoUpload === "true") return;
            const txt = row.innerText || "";
            const u = list.find(x => {
                const name = x.displayName || x.username || x.name || x.email || "";
                return name && txt.includes(name);
            });
            if (!u) return;
            row.dataset.wtPhotoUpload = "true";
            const box = document.createElement("div");
            box.className = "wt-user-photo-upload";
            box.innerHTML = '<label>Profilfoto hochladen<input type="file" accept="image/*"></label><button type="button">Speichern</button><span></span>';
            row.appendChild(box);
            box.querySelector("button").addEventListener("click", async () => {
                const s = box.querySelector("span");
                try {
                    const url = await fileToUrl(box.querySelector("input").files[0]);
                    u.imageUrl = url; u.photoUrl = url; u.avatar = url;
                    await save();
