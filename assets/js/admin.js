        ...
        <div class="p-6 md:p-8">
            ...
            ` : adminTab === 'categories' && hasAdminAccess() ? renderAdminCategoriesTab() : adminTab === 'authors' && hasAdminAccess() ? renderAdminAuthorsTab() : adminTab === 'users' && hasAdminAccess() ? `
                <div class="flex justify-between items-center border-b pb-4 mb-6">
                    <h3 class="text-xl font-bold uppercase flex items-center gap-2"><i data-lucide="users" class="text-blue-600"></i> Registrierte Benutzer (${registeredUsers.filter(u => !u.isDeleted).length})</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ${registeredUsers.filter(u => !u.isDeleted).length === 0 ? '<p class="text-gray-500 italic">Noch keine Benutzer registriert.</p>' : ''}
                    ${registeredUsers.filter(u => !u.isDeleted).map(u => `
                        <div class="border ${u.isBanned || u.isDeleted ? 'border-red-300 bg-red-50' : 'border-gray-200'} rounded p-4 flex flex-col sm:flex-row gap-4 relative">
                            ...
                            <div class="flex-1 mt-2 sm:mt-0">
                                ...
                                <button onclick="permanentlyDeleteUser('${u.username}')" class="mt-2 ml-0 sm:ml-2 bg-red-50 border border-red-200 text-red-700 text-sm font-bold px-4 py-2 rounded hover:bg-red-100 transition-colors w-full sm:w-auto">Endgültig löschen</button>
                            </div>
                        </div>
                    `).join('')}
                </div>
...
window.permanentlyDeleteUser = function(username) {
    ...
    currentModal = {
        ...
        onConfirm: async function() {
            const deletedUser = registeredUsers.find(u => u.username === username) || { username };
            if (typeof rememberDeletedUser === "function") {
                rememberDeletedUser(deletedUser);
            } else if (typeof window.rememberDeletedUser === "function") {
                window.rememberDeletedUser(deletedUser);
            }
