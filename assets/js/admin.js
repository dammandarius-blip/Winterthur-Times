...
function getAdminCategoriesSafe() {
    ...
    const defaults = ["Politik", "Wirtschaft", "Gesellschaft", "Kultur", "Sport", "Lokales", "Wissenschaft", "Unterhaltung", "Panorama"];
    const fromArticles = Array.from(new Set((Array.isArray(articles) ? articles : []).map(a => a && a.category).filter(Boolean)));

    window.categories = fromArticles.length ? fromArticles : defaults;
    ...
    return `
        <div class="flex justify-between items-center border-b pb-4 mb-6">
            <h3 class="text-xl font-bold uppercase flex items-center gap-2">
                <i data-lucide="folder" class="text-blue-600"></i> Ressorts verwalten
            </h3>
        </div>

        <div class="bg-blue-50 border border-blue-100 rounded p-4 mb-6 text-sm text-gray-700">
            Hier verwaltest du die Kategorien, die im Menü und bei Artikeln verwendet werden.
        </div>
        ...
        <div class="flex flex-col gap-3">
            ${cats.map(cat => `
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-white border border-gray-200 rounded p-4">
                    ...
                    <button onclick="deleteAdminCategory('${String(cat).replace(/'/g, "\\'")}')" class="bg-red-50 text-red-700 border border-red-200 px-4 py-2 rounded font-bold hover:bg-red-100 cursor-pointer">
                        Löschen
                    </button>
                </div>
            `).join("")}
...
function renderAdminAuthorsTab() {
    ...
    return `
        <div class="flex justify-between items-center border-b pb-4 mb-6">
            <h3 class="text-xl font-bold uppercase flex items-center gap-2">
                <i data-lucide="user-pen" class="text-blue-600"></i> Autoren verwalten
            </h3>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            ${list.map(author => `
                <div class="bg-white border border-gray-200 rounded p-4 flex gap-4">
                    <img src="${escapeHtml(author.imageUrl || author.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(author.name || 'Autor')}`)}" alt="" class="w-16 h-16 rounded-full object-cover bg-gray-100">
                    <div class="flex-1">
                        ...
                        <div class="flex gap-2 mt-3">
                            <button onclick="editAdminAuthor('${String(author.id || author.name).replace(/'/g, "\\'")}')" class="text-blue-700 bg-blue-50 border border-blue-100 px-3 py-1 rounded font-bold hover:bg-blue-100 cursor-pointer">Bearbeiten</button>
                            <button onclick="deleteAdminAuthor('${String(author.id || author.name).replace(/'/g, "\\'")}')" class="text-red-700 bg-red-50 border border-red-100 px-3 py-1 rounded font-bold hover:bg-red-100 cursor-pointer">Löschen</button>
                        </div>
                    </div>
                </div>
            `).join("")}
...
window.clearAdminAuthorForm = function() {
    const ids = ["authorEditId", "authorName", "authorImageUrl", "authorBio"];
    ids.forEach(id => {
