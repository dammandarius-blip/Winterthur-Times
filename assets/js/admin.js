...
window.handleCreateArticle = async function(event) {
    ...
    const saveArticle = (imgSrc) => {
        if (editingArticleId) {
            ...
            wtSaveArticleAppendOnly(newArticle);
...
(function wtRessortSaveFix() {
    ...
    async function saveRessortsToFirebase() {
        ...
        try {
            ...
            window.dispatchEvent(new CustomEvent("wtCategoriesUpdated", { detail: { categories: cats.slice() } }));
        } catch (_) {}
...
/* =========================================================
   Admin Ressorts: Reihenfolge verschieben und speichern
   ========================================================= */
(function wtRessortOrderMoveFix() {
    if (window.__wtRessortOrderMoveFix) return;
    window.__wtRessortOrderMoveFix = true;
    ...
    async function saveOrder() {
        if (typeof window.wtSaveRessortsToFirebase === "function") {
            await window.wtSaveRessortsToFirebase();
            return;
        }
        ...
        try {
            ...
            console.warn("Ressort-Reihenfolge speichern fehlgeschlagen:", err);
        }
    }
    ...
    window.wtMoveRessort = moveCategory;

    function enhanceRessortRows() {
        // Only on Ressorts tab/panel
        if (!/Ressorts verwalten|RESSORTS VERWALTEN/i.test(document.body.innerText || "")) return;

        const cats = getCategoriesSafe();
        if (!cats.length) return;

        const cards = Array.from(document.querySelectorAll("div, li, tr"))
            .filter(el => {
                const text = (el.innerText || "").trim();
                if (!text) return false;
                return cats.some(cat => text.startsWith(cat) || text.includes("\n" + cat));
            });

        cards.forEach(card => {
            if (card.dataset.wtRessortOrderEnhanced === "true") return;
            const text = card.innerText || "";
            const cat = cats.find(c => text.includes(c));
            if (!cat) return;

            // Avoid enhancing huge containers
            if (card.innerText.length > 250) return;

            card.dataset.wtRessortOrderEnhanced = "true";

            const controls = document.createElement("div");
            controls.className = "wt-ressort-order-controls";
            controls.innerHTML = `
                <button type="button" title="Nach oben" data-wt-move-up="${cat.replace(/"/g, "&quot;")}">↑</button>
                <button type="button" title="Nach unten" data-wt-move-down="${cat.replace(/"/g, "&quot;")}">↓</button>
            `;
    ...
    document.addEventListener("click", event => {
        const up = event.target.closest("[data-wt-move-up]");
        if (up) {
            event.preventDefault();
            event.stopPropagation();
            moveCategory(up.dataset.wtMoveUp, -1);
            return;
        }

        const down = event.target.closest("[data-wt-move-down]");
        if (down) {
            event.preventDefault();
            event.stopPropagation();
            moveCategory(down.dataset.wtMoveDown, 1);
        }
    }, true);

    const observer = new MutationObserver(enhanceRessortRows);
    observer.observe(document.documentElement, { childList: true, subtree: true });

    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", enhanceRessortRows);
    } else {
        enhanceRessortRows();
    }

    setInterval(enhanceRessortRows, 1500);
})();
...
/* =========================================================
   Menü-Freeze: sichtbares Menü wird bei externen Updates nicht neu aufgebaut
   ========================================================= */
(function wtFreezeVisibleMenuOnExternalUpdates() {
    if (window.__wtFreezeVisibleMenuOnExternalUpdates) return;
    window.__wtFreezeVisibleMenuOnExternalUpdates = true;

    let queuedRender = false;
    let renderingQueued = false;

    function menuIsOpen() {
        try {
            if (typeof isMenuOpen !== "undefined" && isMenuOpen) return true;
        } catch (_) {}
        return !!document.querySelector(".wt-mobile-menu-panel-safe, .wt-mobile-menu-fallback, .fixed.inset-0 nav, .fixed.inset-0 .bg-white");
    }

    function patchRenderApp() {
        if (typeof window.renderApp !== "function" || window.renderApp.__wtMenuFreezePatched) return;

        const original = window.renderApp;
        const patched = function(...args) {
            if (menuIsOpen() && !renderingQueued) {
                queuedRender = true;
                return;
            }
        ...
        patched.__wtMenuFreezePatched = true;
        window.renderApp = patched;
    }

    function flushIfClosed() {
        patchRenderApp();
        if (!queuedRender || menuIsOpen()) return;
    ...
    document.addEventListener("click", () => setTimeout(flushIfClosed, 150), true);
    window.addEventListener("hashchange", () => setTimeout(flushIfClosed, 150));
    ...
    if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", patchRenderApp);
    else patchRenderApp();

    setInterval(flushIfClosed, 1000);
})();
