...
/* Ressort Spiele + Artikelschutz */
(function wtAdminGamesAndArticleProtect() {
    if (window.__wtAdminGamesAndArticleProtect) return;
    window.__wtAdminGamesAndArticleProtect = true;

    function ensureGamesCategory() {
        try {
            if (typeof categories !== "undefined" && Array.isArray(categories) && !categories.includes("Spiele")) categories.push("Spiele");
        } catch (_) {}
        if (Array.isArray(window.categories) && !window.categories.includes("Spiele")) window.categories.push("Spiele");
    }

    function removeAutoDeleteDates() {
        try {
            if (typeof articles !== "undefined" && Array.isArray(articles)) articles.forEach(a => { if (a) delete a.autoDeleteDate; });
        } catch (_) {}
        if (Array.isArray(window.articles)) window.articles.forEach(a => { if (a) delete a.autoDeleteDate; });
    }

    ensureGamesCategory();
    removeAutoDeleteDates();
    setInterval(() => { ensureGamesCategory(); removeAutoDeleteDates(); }, 3000);
})();
