...
/* =========================================================
   Fix: Admin-Bereiche "Ressorts" und "Autoren"
   ========================================================= */
(function fixAdminRessortsAndAuthorsPanels() {
    if (window.__wtRessortsAuthorsFixInstalled) return;
    window.__wtRessortsAuthorsFixInstalled = true;

    const DEFAULT_CATEGORIES = ["Politik", "Wirtschaft", "Gesellschaft", "Kultur", "Sport", "Lokales", "Wissenschaft", "Unterhaltung", "Panorama"];

    function getAdminRoot() {
        return document.querySelector("#adminPanel")
    ...
    function getCategoriesArray() {
        ...
        const fromArticles = Array.from(new Set(getArticlesArray().map(a => a && a.category).filter(Boolean)));
        return fromArticles.length ? fromArticles : DEFAULT_CATEGORIES.slice();
    }
    ...
    async function saveAllData() {
        try {
            ...
            console.warn("saveData fehlgeschlagen:", err);
        }
        ...
        try {
            ...
            console.error("Direktes Firebase-Speichern fehlgeschlagen:", err);
            throw err;
        }
    }
    ...
    function ensureFixPanel() {
        let panel = document.getElementById("wtAdminFallbackPanel");
        if (panel) return panel;

        panel = document.createElement("section");
        panel.id = "wtAdminFallbackPanel";
        panel.className = "wt-admin-fallback-panel";
        panel.style.display = "none";

        getAdminRoot().appendChild(panel);
        return panel;
    }

    function showOnlyFixPanel() {
        const panel = ensureFixPanel();

        // Hide common admin content cards without destroying them.
        Array.from(document.querySelectorAll(".admin-section, .admin-tab-content, .tab-content, [data-admin-section], [data-tab-content]")).forEach(el => {
            if (el !== panel && !panel.contains(el)) el.style.display = "none";
        });
    ...
    function renderRessortsPanel() {
        const panel = showOnlyFixPanel();
        const cats = getCategoriesArray();
        const articleCounts = getArticlesArray().reduce((acc, article) => {
        ...
        panel.innerHTML = `
            <div class="wt-admin-fallback-card">
                <div class="wt-admin-fallback-head">
                    <div>
                        <p class="wt-admin-kicker">Admin</p>
                        <h2>Ressorts verwalten</h2>
                        <p>Hier kannst du die Kategorien/Ressorts prüfen und neue hinzufügen.</p>
                    </div>
                    <button type="button" class="wt-admin-secondary" data-wt-admin-back>Zurück</button>
                </div>

                <div class="wt-admin-inline-form">
                    <input id="wtNewCategoryName" type="text" placeholder="Neues Ressort, z. B. Bildung">
                    <button type="button" class="wt-admin-primary" id="wtAddCategoryBtn">Ressort hinzufügen</button>
                </div>

                <div class="wt-admin-list">
                    ${cats.map(cat => `
                        <div class="wt-admin-list-row">
                            <div>
                                <strong>${escapeHtml(cat)}</strong>
                                <span>${articleCounts[cat] || 0} Artikel</span>
                            </div>
                            <button type="button" class="wt-admin-danger" data-wt-delete-category="${escapeHtml(cat)}">Löschen</button>
                        </div>
                    `).join("")}
                </div>

                <p id="wtRessortsStatus" class="wt-admin-status"></p>
            </div>
        `;

        panel.querySelector("[data-wt-admin-back]")?.addEventListener("click", () => {
            panel.style.display = "none";
            location.reload();
        });

        panel.querySelector("#wtAddCategoryBtn")?.addEventListener("click", async () => {
            const input = panel.querySelector("#wtNewCategoryName");
            const name = (input?.value || "").trim();
            const status = panel.querySelector("#wtRessortsStatus");

            if (!name) {
                if (status) status.textContent = "Bitte einen Ressort-Namen eingeben.";
                return;
            }

            const catsRef = getCategoriesArray();
            if (!catsRef.includes(name)) {
                try {
                    if (typeof categories !== "undefined" && Array.isArray(categories)) {
                        categories.push(name);
                    } else {
                        window.categories = catsRef.concat(name);
                    }
                    await saveAllData();
                    renderRessortsPanel();
                } catch (err) {
                    if (status) status.textContent = "Speichern fehlgeschlagen: " + err.message;
                }
            }
        });

        panel.querySelectorAll("[data-wt-delete-category]").forEach(btn => {
            btn.addEventListener("click", async () => {
                const name = btn.dataset.wtDeleteCategory;
                const status = panel.querySelector("#wtRessortsStatus");

                if ((articleCounts[name] || 0) > 0) {
                    if (status) status.textContent = "Dieses Ressort enthält noch Artikel und kann deshalb nicht gelöscht werden.";
                    return;
                }
                ...
                try {
                    if (typeof categories !== "undefined" && Array.isArray(categories)) {
                        ...
                        window.categories = getCategoriesArray().filter(cat => cat !== name);
                    }
                    await saveAllData();
                    renderRessortsPanel();
                } catch (err) {
                    if (status) status.textContent = "Löschen fehlgeschlagen: " + err.message;
                }
            });
        });
    }
    ...
    function renderAuthorsPanel() {
        ...
        panel.innerHTML = `
            <div class="wt-admin-fallback-card">
                <div class="wt-admin-fallback-head">
                    <div>
                        <p class="wt-admin-kicker">Admin</p>
                        <h2>Autoren verwalten</h2>
                        <p>Hier kannst du Autoren anlegen, bearbeiten und löschen.</p>
                    </div>
                    <button type="button" class="wt-admin-secondary" data-wt-admin-back>Zurück</button>
                </div>

                <div class="wt-admin-author-form">
                    <input id="wtAuthorName" type="text" placeholder="Name">
                    <input id="wtAuthorImage" type="url" placeholder="Bild-URL optional">
                    <textarea id="wtAuthorBio" rows="3" placeholder="Kurzbeschreibung/Bio"></textarea>
                    <button type="button" class="wt-admin-primary" id="wtAddAuthorBtn">Autor speichern</button>
                </div>

                <div class="wt-admin-list">
                    ${list.map(author => `
                        <div class="wt-admin-list-row wt-admin-author-row">
                            <img src="${escapeHtml(author.imageUrl || author.image || "https://ui-avatars.com/api/?name=" + encodeURIComponent(author.name || "Autor"))}" alt="" loading="lazy">
                            <div>
                                <strong>${escapeHtml(author.name || "Unbenannter Autor")}</strong>
                                <span>${escapeHtml(author.bio || "Keine Bio hinterlegt.")}</span>
                                <small>${articleCounts[author.name] || 0} Artikel</small>
                            </div>
                            <div class="wt-admin-row-actions">
                                <button type="button" class="wt-admin-secondary" data-wt-edit-author="${escapeHtml(author.id || author.name)}">Bearbeiten</button>
                                <button type="button" class="wt-admin-danger" data-wt-delete-author="${escapeHtml(author.id || author.name)}">Löschen</button>
                            </div>
                        </div>
                    `).join("") || `<p>Noch keine Autoren angelegt.</p>`}
                </div>

                <p id="wtAuthorsStatus" class="wt-admin-status"></p>
            </div>
        `;
        ...
        panel.querySelector("#wtAddAuthorBtn")?.addEventListener("click", async () => {
            const status = panel.querySelector("#wtAuthorsStatus");
            const name = (panel.querySelector("#wtAuthorName")?.value || "").trim();
            const imageUrl = (panel.querySelector("#wtAuthorImage")?.value || "").trim();
            const bio = (panel.querySelector("#wtAuthorBio")?.value || "").trim();

            if (!name) {
                if (status) status.textContent = "Bitte einen Namen eingeben.";
                return;
            }

            const authorsRef = getAuthorsArray();
            const newAuthor = {
                id: editingAuthorId || ("author_" + Date.now()),
                name,
                bio,
                imageUrl
            };

            const idx = authorsRef.findIndex(a => String(a.id || a.name) === String(editingAuthorId));
            if (idx >= 0) {
                authorsRef[idx] = newAuthor;
            } else {
                authorsRef.push(newAuthor);
            }

            try {
                if (typeof authors === "undefined" || !Array.isArray(authors)) window.authors = authorsRef;
                await saveAllData();
                renderAuthorsPanel();
            } catch (err) {
                if (status) status.textContent = "Speichern fehlgeschlagen: " + err.message;
        ...
        panel.querySelectorAll("[data-wt-edit-author]").forEach(btn => {
            btn.addEventListener("click", () => {
                const id = btn.dataset.wtEditAuthor;
                const author = getAuthorsArray().find(a => String(a.id || a.name) === String(id));
                if (!author) return;

                editingAuthorId = author.id || author.name;
                panel.querySelector("#wtAuthorName").value = author.name || "";
                panel.querySelector("#wtAuthorImage").value = author.imageUrl || author.image || "";
                panel.querySelector("#wtAuthorBio").value = author.bio || "";
                panel.querySelector("#wtAuthorName").focus();
            });
        });

        panel.querySelectorAll("[data-wt-delete-author]").forEach(btn => {
            btn.addEventListener("click", async () => {
                const id = btn.dataset.wtDeleteAuthor;
                const authorsRef = getAuthorsArray();
                const author = authorsRef.find(a => String(a.id || a.name) === String(id));

                if (author && (articleCounts[author.name] || 0) > 0) {
                    const status = panel.querySelector("#wtAuthorsStatus");
                    if (status) status.textContent = "Dieser Autor hat noch Artikel und kann deshalb nicht gelöscht werden.";
                    return;
                }

                const idx = authorsRef.findIndex(a => String(a.id || a.name) === String(id));
                if (idx >= 0) authorsRef.splice(idx, 1);
    ...
    function textLooksLike(element, words) {
        const text = (element.textContent || "").trim().toLowerCase();
        return words.some(word => text === word || text.includes(word));
    }

    function bindAdminButtons() {
        Array.from(document.querySelectorAll("button, a, [role='tab'], [onclick], .tab, .admin-tab")).forEach(el => {
            if (el.dataset.wtRessortsAuthorsFixed === "true") return;

            if (textLooksLike(el, ["ressorts", "kategorien"])) {
                el.dataset.wtRessortsAuthorsFixed = "true";
            ...
            if (textLooksLike(el, ["autoren", "authors"])) {
                el.dataset.wtRessortsAuthorsFixed = "true";
                el.addEventListener("click", event => {
                    event.preventDefault();
                    event.stopPropagation();
    ...
    const observer = new MutationObserver(bindAdminButtons);
    observer.observe(document.documentElement, { childList: true, subtree: true });

    window.wtRenderAdminRessortsPanel = renderRessortsPanel;
    window.wtRenderAdminAuthorsPanel = renderAuthorsPanel;
})();
