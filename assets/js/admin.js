...
/* Kein automatisches Artikel-Löschen */
(function wtNoAutoDeleteArticles() {
    if (window.__wtNoAutoDeleteArticles) return;
    window.__wtNoAutoDeleteArticles = true;

    function clean() {
        try {
            if (typeof articles !== "undefined" && Array.isArray(articles)) {
                articles.forEach(article => { if (article) delete article.autoDeleteDate; });
            }
        } catch (_) {}
        if (Array.isArray(window.articles)) {
            window.articles.forEach(article => { if (article) delete article.autoDeleteDate; });
        }
    }
