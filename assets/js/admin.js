...
/* =========================================================
   Artikel-Adminliste Sofort-Update Fix
   ========================================================= */
(function wtArticleAdminListImmediateFix() {
    if (window.__wtArticleAdminListImmediateFix) return;
    window.__wtArticleAdminListImmediateFix = true;

    function articleKey(article) {
        if (!article) return "";
        return String(article.id || article.title || "").trim();
    }

    function getArticleArrays() {
        const arrays = [];
        try {
            if (typeof articles !== "undefined" && Array.isArray(articles)) arrays.push(articles);
        } catch (_) {}
        if (Array.isArray(window.articles) && !arrays.includes(window.articles)) arrays.push(window.articles);
        return arrays;
    }

    function sortArticles(list) {
        return (Array.isArray(list) ? list : []).sort((a, b) => {
            const bt = new Date(b && b.timestamp ? b.timestamp : 0).getTime() || 0;
            const at = new Date(a && a.timestamp ? a.timestamp : 0).getTime() || 0;
            return bt - at;
        });
    }

    window.getAdminArticleListSafe = function() {
        const main = getArticleArrays()[0] || [];
        const list = main.slice();
        return sortArticles(typeof window.wtFilterDeletedArticlesHard === "function" ? window.wtFilterDeletedArticlesHard(list) : list);
    };

    window.wtEnsureArticleInAdminList = function(article) {
        if (!article || typeof article !== "object") return false;
        const key = articleKey(article);
        if (!key) return false;

        let changed = false;
        const arrays = getArticleArrays();

        if (!arrays.length) {
            window.articles = [article];
            changed = true;
        }

        arrays.forEach(arr => {
            const idx = arr.findIndex(a => articleKey(a) === key);
            if (idx >= 0) {
                arr[idx] = { ...arr[idx], ...article };
            } else {
                arr.unshift(article);
                changed = true;
            }
        ...
        try {
            const current = getArticleArrays()[0] || window.articles || [];
            localStorage.setItem("wt_articles_backup_v2", JSON.stringify(current));
            localStorage.setItem("wt_articles_admin_last_save", JSON.stringify({
                savedAt: Date.now(),
                article
            }));
        } catch (_) {}
    ...
    window.wtRefreshAdminArticleListNow = function(article) {
        if (article) window.wtEnsureArticleInAdminList(article);

        try {
            if (typeof adminTab !== "undefined") adminTab = "articles";
        } catch (_) {}
        ...
        if (typeof renderApp === "function") {
            setTimeout(() => {
                try {
                    ...
                    console.warn("Admin-Artikelliste konnte nicht neu gerendert werden:", err);
                }
            }, 120);
        }
    };

    window.addEventListener("wtArticleSaved", event => {
        window.wtRefreshAdminArticleListNow(event.detail && event.detail.article ? event.detail.article : null);
    });
})();
...
async function wtSaveArticleAppendOnly(newArticle) {
    ...
    try {
        if (typeof window.wtEnsureArticleInAdminList === "function" && newArticle) window.wtEnsureArticleInAdminList(newArticle);
        window.dispatchEvent(new CustomEvent("wtArticleSaved", { detail: { article: newArticle, articles: mergedArticles } }));
    } catch (_) {}
...
window.renderAdminDashboard = function() {
    ...
    <div class="max-w-6xl mx-auto bg-white shadow-md rounded-sm font-sans mt-4">
        ...
        <div class="p-6 md:p-8">
            ${adminTab === 'articles' ? `
                <div id="wt-advanced-article-editor-host" class="mb-8">
                    <div class="border border-gray-200 bg-gray-50 p-6 text-sm text-gray-600">
                        Artikel-Editor wird geladen...
                    </div>
                </div>
                ...
                <div class="mt-12 border-t pt-8">
                    ...
                    <div class="flex flex-col gap-4">
                        ${getAdminArticleListSafe().length === 0 ? '<p class="text-gray-500 italic">Keine Artikel vorhanden.</p>' : ''}
                        ${getAdminArticleListSafe().map(a => {
            ...
            ` : adminTab === 'categories' && hasAdminAccess() ? renderAdminCategoriesTab() : adminTab === 'authors' && hasAdminAccess() ? renderAdminAuthorsTab() : adminTab === 'users' && hasAdminAccess() ? `
                <div class="flex justify-between items-center border-b pb-4 mb-6">
                    <h3 class="text-xl font-bold uppercase flex items-center gap-2"><i data-lucide="users" class="text-blue-600"></i> Registrierte Benutzer (${(typeof window.wtShouldShowFirebaseUserLoading === "function" && window.wtShouldShowFirebaseUserLoading()) ? "..." : registeredUsers.length})</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ${(typeof window.wtShouldShowFirebaseUserLoading === "function" && window.wtShouldShowFirebaseUserLoading()) ? '<p class="text-gray-500 italic">Benutzer werden aus Firebase geladen...</p>' : ''}
                    ${(!(typeof window.wtShouldShowFirebaseUserLoading === "function" && window.wtShouldShowFirebaseUserLoading()) && registeredUsers.length === 0) ? '<p class="text-gray-500 italic">Noch keine Benutzer registriert.</p>' : ''}
                    ${(typeof window.wtShouldShowFirebaseUserLoading === "function" && window.wtShouldShowFirebaseUserLoading()) ? '' : registeredUsers.map(u => `
...
window.toggleChatAi = async function(chatId) {
    ...
    try {
        const res = await fetch("https://askai.mikestaub705.workers.dev/api/admin/ai", {
            ...
            body: JSON.stringify({
                password: requireAdminSessionPassword(),
                chatId: chatId,
        ...
        await refreshAdminSupportChatsFromWorker(requireAdminSessionPassword(), true);
    } catch (err) {
...
window.deleteArticle = function(id) {
    ...
    const articleToDelete =
        (typeof window.getAdminArticleListSafe === "function" ? window.getAdminArticleListSafe() : articles)
            .find(a => String(a.id) === String(id));

    currentModal = {
        title: 'Artikel löschen?',
        message: articleToDelete
            ? `Möchtest du den Artikel "${articleToDelete.title}" wirklich dauerhaft löschen?`
            : 'Möchtest du diesen Artikel wirklich dauerhaft löschen?',
        onConfirm: async function() {
            try {
                ...
                try {
                    const lastRaw = localStorage.getItem("wt_articles_admin_last_save");
                    if (lastRaw && articleToDelete) {
                        const last = JSON.parse(lastRaw);
                        const lastArticle = last && last.article;
                        const same =
                            lastArticle &&
                            (String(lastArticle.id) === String(articleToDelete.id) ||
                             String(lastArticle.title || "").trim().toLowerCase() === String(articleToDelete.title || "").trim().toLowerCase());
                        if (same) localStorage.removeItem("wt_articles_admin_last_save");
                    }
                } catch (_) {}

                const filterFn = typeof window.wtFilterDeletedArticlesHard === "function"
                    ? window.wtFilterDeletedArticlesHard
                    : (list) => list.filter(a => String(a.id) !== String(id));

                articles = filterFn((Array.isArray(articles) ? articles : []).filter(a => String(a.id) !== String(id)));
                window.articles = articles;

                if (typeof window.wtPersistArticlesSafely === "function") {
                    articles = await window.wtPersistArticlesSafely(articles);
                    window.articles = articles;
                } else {
                    window.saveState();
                }
                ...
                currentModal = {
                    title: 'Gelöscht',
                    message: 'Der Artikel wurde dauerhaft aus der Admin-Liste, dem Backup und der Datenbank entfernt.'
                };
                ...
                currentModal = null;
                showModal("Fehler", "Der Artikel konnte nicht dauerhaft gelöscht werden: " + (err && err.message ? err.message : err));
                renderApp();
            }
        }
    };
...
window.toggleUserBan = async function(username) {
    if(!hasAdminAccess()) return;
    const user = registeredUsers.find(u => u.username === username);
    if (user) {
        user.isBanned = !user.isBanned;
        try {
            if (typeof window.saveUsersNow === "function") await window.saveUsersNow();
            else window.saveState();
        } catch (err) {
            console.error("Benutzersperre konnte nicht sofort gespeichert werden:", err);
            window.saveState();
        }
        renderApp();
    }
...
window.toggleUserDeleted = async function(username) {
    if(!hasAdminAccess()) return;
    const user = registeredUsers.find(u => u.username === username);
    if (user) {
        ...
        try {
            ...
            console.error("Benutzer-Loeschstatus konnte nicht sofort gespeichert werden:", err);
            window.saveState();
        }
        renderApp();
    }
...
window.permanentlyDeleteUser = function(username) {
    ...
    currentModal = {
        ...
        onConfirm: async function() {
            ...
            try {
                if (typeof window.saveUsersNow === "function") await window.saveUsersNow();
                else if (typeof saveUsersNow === "function") await saveUsersNow();
                else window.saveState();
            } catch (err) {
                console.error("Benutzer konnte nicht sofort gespeichert werden:", err);
                window.saveState();
            }
            renderApp();
...
window.exportArticles = function() {
    const dataStr = JSON.stringify(getAdminArticleListSafe(), null, 2);
...
window.exportBackup = function() {
    const backupData = {
        articles: getAdminArticleListSafe(),
        authors: authors,
...
const WT_ADMIN_SESSION_PASSWORD_KEY = "wt_admin_worker_password";
...
function setAdminSessionPassword(password) {
    const clean = String(password || "").trim();
    try {
        if (clean) sessionStorage.setItem(WT_ADMIN_SESSION_PASSWORD_KEY, clean);
        else sessionStorage.removeItem(WT_ADMIN_SESSION_PASSWORD_KEY);
    } catch (_) {}
}

function getAdminSessionPassword() {
    try {
        return String(sessionStorage.getItem(WT_ADMIN_SESSION_PASSWORD_KEY) || "").trim();
    } catch (_) {
        return "";
    }
}

function requireAdminSessionPassword() {
    const password = getAdminSessionPassword();
    if (!password) {
        isSuperAdmin = false;
        if (typeof showModal === "function") {
            showModal("Admin-Login erforderlich", "Bitte melde dich neu an. Das Admin-Passwort wird nicht mehr im Website-Code gespeichert.");
        }
        throw new Error("Admin session password missing");
    }
    return password;
}
...
window.handleLogin = async function (event) {
    ...
    try {
        ...
        isSuperAdmin = false;
        if (errorEl) {
            errorEl.textContent = "Login fehlgeschlagen. Passwort oder Worker-Konfiguration pruefen.";
            errorEl.classList.remove("hidden");
        }
    }
};
...
/* Fallback: zuletzt gespeicherten Artikel in Adminliste wieder einsetzen */
(function wtRestoreLastSavedArticleInAdminList() {
    if (window.__wtRestoreLastSavedArticleInAdminList) return;
    window.__wtRestoreLastSavedArticleInAdminList = true;

    function key(a) { return String(a && (a.id || a.title) || ""); }

    setInterval(() => {
        try {
            const raw = localStorage.getItem("wt_articles_admin_last_save");
            if (!raw) return;
            const data = JSON.parse(raw);
            const article = data && data.article;
            if (!article) return;
            const list = typeof window.getAdminArticleListSafe === "function" ? window.getAdminArticleListSafe() : (Array.isArray(window.articles) ? window.articles : []);
            if (typeof window.wtArticleIsDeletedHard === "function" && window.wtArticleIsDeletedHard(article)) {
                localStorage.removeItem("wt_articles_admin_last_save");
                return;
            }
            if (!list.some(a => key(a) === key(article))) {
                if (typeof window.wtEnsureArticleInAdminList === "function") window.wtEnsureArticleInAdminList(article);
                if (typeof adminTab !== "undefined" && adminTab === "articles" && typeof renderApp === "function") renderApp();
            }
        } catch (_) {}
    }, 1500);
})();
...
/* Admin: gelöschte Artikel nicht wiederherstellen */
(function wtAdminDeletedArticleGuard() {
    if (window.__wtAdminDeletedArticleGuard) return;
    window.__wtAdminDeletedArticleGuard = true;

    setInterval(() => {
        try {
            if (typeof window.wtFilterDeletedArticlesHard !== "function") return;
            const clean = window.wtFilterDeletedArticlesHard(Array.isArray(window.articles) ? window.articles : []);
            if (Array.isArray(window.articles) && clean.length !== window.articles.length) {
                window.articles.splice(0, window.articles.length, ...clean);
                try { articles = window.articles; } catch (_) {}
                if (typeof adminTab !== "undefined" && adminTab === "articles" && typeof renderApp === "function") renderApp();
            }
        } catch (_) {}
    }, 1000);
