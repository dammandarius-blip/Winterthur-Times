...
/* =========================================================
   Solitaire Spiel – Startseiten Widget
   ========================================================= */
(function initWinterthurTimesSolitaire() {
    if (window.__wtSolitaireInstalled) return;
    window.__wtSolitaireInstalled = true;
    ...
    function newGame() {
        ...
        moveCount = 0;
        saveGame();
        renderAllSolitaireWidgets();
    }

    function saveGame() {
        try {
            localStorage.setItem("wt_solitaire_state", JSON.stringify({ state, moveCount }));
        } catch (_) {}
    }

    function loadGame() {
        try {
            const raw = localStorage.getItem("wt_solitaire_state");
            if (!raw) return false;
            const parsed = JSON.parse(raw);
            if (!parsed || !parsed.state || !Array.isArray(parsed.state.tableau)) return false;
            state = parsed.state;
    ...
    function canPlaceOnTableau(card, targetCard) {
        ...
        return card.value === targetCard.value - 1 && isRed(card) !== isRed(targetCard);
    }
    ...
    function flipTopIfNeeded(colIndex) {
    ...
    function removeSource(sel) {
        ...
        if (sel.type === "waste") {
            ...
            state.tableau[sel.col].splice(sel.index);
            flipTopIfNeeded(sel.col);
    ...
    function tryMoveToTableau(targetCol) {
        ...
        if (!canPlaceOnTableau(first, targetTop)) return false;
    ...
    function autoMoveCard(sel) {
        selected = sel;
        const cards = getSourceCards(selected);
        if (cards.length !== 1) return false;
        const card = cards[0];

        if (tryMoveToFoundation(card.suit)) return true;

        for (let i = 0; i < 7; i++) {
            if (sel.type === "tableau" && sel.col === i) continue;
            if (tryMoveToTableau(i)) return true;
            selected = sel;
        }
    ...
    function selectCard(sel) {
        ...
        if (selected) {
            ...
            if (JSON.stringify(selected) === JSON.stringify(sel)) {
                selected = null;
                renderAllSolitaireWidgets();
                return;
            }

            // If user selects another tableau card while something is selected, attempt move to that column.
            if (sel.type === "tableau") {
                if (tryMoveToTableau(sel.col)) return;
            }

            if (sel.type === "foundation") {
                if (tryMoveToFoundation(sel.suit)) return;
            }
        }

        const cards = getSourceCards(sel);
    ...
    function checkWin() {
        const done = Object.values(state.foundations).reduce((sum, pile) => sum + pile.length, 0);
        if (done === 52) {
            setTimeout(() => {
                alert(`Gewonnen! Neues Solitaire wird gestartet. Züge: ${moveCount}`);
                newGame();
            }, 250);
        }
    }

    function cardHtml(card, attrs = "", extraClass = "") {
        if (!card) return `<div class="wt-sol-card wt-sol-empty" ${attrs}></div>`;
        if (!card.faceUp) return `<button type="button" class="wt-sol-card wt-sol-back ${extraClass}" ${attrs}>WT</button>`;
        return `
            <button type="button" class="wt-sol-card ${card.color === "red" ? "red" : "black"} ${extraClass}" ${attrs}>
                <span>${card.rank}</span>
    ...
    function renderSolitaire(container, fullscreen = false) {
        if (!state && !loadGame()) newGame();
        if (!state) return;

        const foundationHtml = SUITS.map(suit => {
            const pile = state.foundations[suit];
            const top = pile[pile.length - 1];
            return `
                <div class="wt-sol-foundation" data-sol-foundation="${suit}">
                    ${top ? cardHtml(top, `data-sol-foundation="${suit}"`) : `<button type="button" class="wt-sol-card wt-sol-empty" data-sol-foundation="${suit}">${suit}</button>`}
                </div>
            `;
        }).join("");

        const tableauHtml = state.tableau.map((col, colIndex) => {
            const cards = col.map((card, index) => {
                const isSelected = selected && selected.type === "tableau" && selected.col === colIndex && selected.index === index;
                return `
                    <div class="wt-sol-tableau-card" style="top:${index * (fullscreen ? 30 : 24)}px">
            ...
            return `
                <div class="wt-sol-column" data-sol-column="${colIndex}" style="min-height:${fullscreen ? 360 : 250}px">
                    ${cards || `<button type="button" class="wt-sol-card wt-sol-empty" data-sol-column="${colIndex}">K</button>`}
                </div>
            `;
        }).join("");

        const wasteTop = state.waste[state.waste.length - 1];
        const wasteSelected = selected && selected.type === "waste";

        container.innerHTML = `
            <section class="wt-solitaire-card ${fullscreen ? "fullscreen" : ""}">
                <div class="wt-solitaire-head">
                    <div>
                        <h3>Solitaire</h3>
                        <p>Lege Karten abwechselnd rot/schwarz ab. Asse kommen nach oben.</p>
                    </div>
                    <div class="wt-solitaire-actions">
                        ${fullscreen ? "" : `<button type="button" class="wt-sol-icon" data-sol-fullscreen title="Vollbild">⛶</button>`}
                        <button type="button" class="wt-sol-small" data-sol-new>Neu</button>
                    </div>
                </div>

                <div class="wt-sol-status">
                    <span>Züge: ${moveCount}</span>
                    <span>Stapel: ${state.stock.length}</span>
                </div>

                <div class="wt-sol-top">
                    <button type="button" class="wt-sol-stock wt-sol-card ${state.stock.length ? "wt-sol-back" : "wt-sol-empty"}" data-sol-draw>
                        ${state.stock.length ? "WT" : "↻"}
                    </button>
                    <div class="wt-sol-waste">
                        ${wasteTop ? cardHtml(wasteTop, `data-sol-waste`, wasteSelected ? "selected" : "") : `<div class="wt-sol-card wt-sol-empty"></div>`}
                    </div>
                    <div class="wt-sol-foundations">
                        ${foundationHtml}
                    </div>
                </div>

                <div class="wt-sol-tableau">
                    ${tableauHtml}
                </div>

                <p class="wt-sol-help">Tipp: Karte anklicken und danach Zielstapel anklicken. Doppelklick versucht automatisch abzulegen.</p>
            </section>
        `;
    }

    function renderAllSolitaireWidgets() {
        document.querySelectorAll("[data-wt-solitaire-root]").forEach(root => {
            renderSolitaire(root, root.dataset.fullscreen === "true");
        });
    }

    function openFullscreen() {
        let overlay = document.getElementById("wtSolitaireFullscreen");
        if (!overlay) {
            overlay = document.createElement("div");
            overlay.id = "wtSolitaireFullscreen";
            overlay.className = "wt-sol-fullscreen-overlay";
            overlay.innerHTML = `
                <div class="wt-sol-fullscreen-panel">
                    <button type="button" class="wt-sol-close" data-sol-close>×</button>
                    <div data-wt-solitaire-root data-fullscreen="true"></div>
                </div>
            `;
    ...
    function closeFullscreen() {
        document.getElementById("wtSolitaireFullscreen")?.classList.remove("open");
    }

    function mountWidget() {
        if (document.querySelector("[data-wt-solitaire-root]:not([data-fullscreen='true'])")) return;

        const root = document.createElement("div");
        root.setAttribute("data-wt-solitaire-root", "");
        root.className = "wt-solitaire-widget";

        const crossword =
            document.querySelector(".crossword-card")
            || Array.from(document.querySelectorAll("section,div,article")).find(el => (el.textContent || "").includes("Kreuzworträtsel"));

        if (crossword && crossword.parentNode) {
            crossword.insertAdjacentElement("afterend", root);
        } else {
            const app = document.getElementById("app") || document.querySelector("main") || document.body;
            app.appendChild(root);
        }

        renderSolitaire(root, false);
    }
    ...
    document.addEventListener("click", event => {
        ...
        const fs = event.target.closest("[data-sol-fullscreen]");
        if (fs) {
            openFullscreen();
            return;
        }

        const close = event.target.closest("[data-sol-close]");
        if (close) {
            closeFullscreen();
            return;
        }
        ...
        if (foundation && foundation.dataset.solFoundation) {
            if (selected) moveSelectedToFoundation(foundation.dataset.solFoundation);
            return;
        }
        ...
        if (tableCard) {
            ...
            if (!card.faceUp && index === state.tableau[col].length - 1) {
                card.faceUp = true;
                moveCount++;
                saveGame();
                renderAllSolitaireWidgets();
            ...
            selectCard({ type: "tableau", col, index });
            return;
        }

        const emptyColumn = event.target.closest("[data-sol-column]");
        if (emptyColumn) {
            const col = Number(emptyColumn.dataset.solColumn);
            if (selected) tryMoveToTableau(col);
            return;
        }
    ...
    document.addEventListener("dblclick", event => {
        ...
        if (tableCard) {
            const col = Number(tableCard.dataset.solTableau);
            const index = Number(tableCard.dataset.solIndex);
            autoMoveCard({ type: "tableau", col, index });
            return;
        }

        const waste = event.target.closest("[data-sol-waste]");
        if (waste) {
            autoMoveCard({ type: "waste" });
        }
    });
