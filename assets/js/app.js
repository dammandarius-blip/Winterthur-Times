...
function wtEscapeJsString(value) {
    return String(value ?? "")
        .replace(/\\/g, "\\\\")
        .replace(/'/g, "\\'")
        .replace(/\r/g, "\\r")
...
let pendingRemoteRenderTimer = null;
let authModalDraft = null;
let pendingAuthIdentifier = "";
let hasLoadedRemoteUsers = false;
let wtLastRenderedAppHtml = "";
let wtLastRenderStartedAt = 0;
let wtQueuedRenderTimer = null;
...
function requestRemoteRender() {
    ...
    pendingRemoteRender = true;
    if (pendingRemoteRenderTimer) clearTimeout(pendingRemoteRenderTimer);
    pendingRemoteRenderTimer = setTimeout(() => {
        pendingRemoteRenderTimer = null;
        if (wtShouldDeferRemoteRender()) return;
...
window.wtFlushDeferredRemoteRender = function() {
    ...
    if (pendingRemoteRenderTimer) {
        clearTimeout(pendingRemoteRenderTimer);
        pendingRemoteRenderTimer = null;
    }
    pendingRemoteRender = false;
    renderApp();
...
function getStableUserKeys(user) {
    if (!user || typeof user !== 'object') return [];
    return [
        user.uid,
        user.id,
...
const WT_USER_DELETED_KEY = "wt_deleted_user_keys_v1";
let deletedUserKeys = [];
try { localStorage.removeItem(WT_USER_DELETED_KEY); } catch (_) {}

function readDeletedUserKeys() {
    try {
        const parsed = JSON.parse(localStorage.getItem(WT_USER_DELETED_KEY) || "[]");
...
function writeDeletedUserKeys(keys) {
    deletedUserKeys = Array.from(new Set((keys || []).map(String).filter(Boolean)));
    try { localStorage.setItem(WT_USER_DELETED_KEY, JSON.stringify(deletedUserKeys)); } catch (_) {}
}

function getDeletedUserKeys() {
    if (!deletedUserKeys.length) deletedUserKeys = readDeletedUserKeys();
    return deletedUserKeys.slice();
}

function rememberDeletedUser(userOrUsername) {
    const keys = typeof userOrUsername === "string"
        ? [String(userOrUsername).trim().toLowerCase()]
        : getStableUserKeys(userOrUsername);
    writeDeletedUserKeys([...getDeletedUserKeys(), ...keys]);
}

window.rememberDeletedUser = rememberDeletedUser;
window.getDeletedUserKeys = getDeletedUserKeys;
...
function mergeRegisteredUsersWithoutAutoDelete(localUsers, remoteUsers) {
    ...
    if (remote.length === 0 && local.length > 0) {
        return local.map(user => ensureUserSubscriptions({ ...user }));
    }
...
let latestRemoteUserRolesMap = null;
...
function applySavedUserRolesToRegisteredUsers(remoteRolesMap) {
    if (remoteRolesMap && typeof remoteRolesMap === "object") {
        latestRemoteUserRolesMap = remoteRolesMap;
        return applyUserRolesMapToUsers(registeredUsers, remoteRolesMap);
    }
    if (isFirebaseConnected) {
        if (latestRemoteUserRolesMap && typeof latestRemoteUserRolesMap === "object") {
            return applyUserRolesMapToUsers(registeredUsers, latestRemoteUserRolesMap);
        }
        return false;
    }
    return applyUserRolesMapToUsers(registeredUsers, loadLocalUserRolesMap());
}
...
window.wtHasLoadedRemoteUsers = function() {
    return !isFirebaseConnected || hasLoadedRemoteUsers;
};
window.wtShouldShowFirebaseUserLoading = function() {
    return !!(isFirebaseConnected && !hasLoadedRemoteUsers);
};

function applyRemoteUsersDoc(data) {
    const remoteUsers = Array.isArray(data && data.registeredUsers) ? data.registeredUsers : [];
    registeredUsers = remoteUsers
        .filter(Boolean)
        .map(user => ensureUserSubscriptions({ ...user }));
    window.registeredUsers = registeredUsers;
    hasLoadedRemoteUsers = true;

    if (data && data.userRoles && typeof data.userRoles === "object") {
        applySavedUserRolesToRegisteredUsers(data.userRoles);
    } else {
        latestRemoteUserRolesMap = null;
    }
...
async function loadUsersFromFirebaseOnce() {
    if (!isFirebaseConnected || !firebaseDb) return false;
    const snap = await firebaseDb.collection("data").doc("users").get();
    const data = snap.exists ? (snap.data() || {}) : {};
    isApplyingRemoteState = true;
    try {
        applyRemoteUsersDoc(data);
    } finally {
...
async function saveUsersNow() {
    ...
    if (isFirebaseConnected && firebaseDb && typeof firebase !== "undefined") {
        if (!hasLoadedRemoteUsers) {
            await loadUsersFromFirebaseOnce();
            ensureAllUsersSubscriptions();
        }
...
async function ensureAuthUserProfile(authUser, preferredUsername = "") {
    ...
    if (!Array.isArray(registeredUsers)) registeredUsers = [];
    if (isFirebaseConnected && firebaseDb && !hasLoadedRemoteUsers) {
        try {
            await loadUsersFromFirebaseOnce();
        } catch (err) {
            console.warn("Firebase-Benutzer konnten vor Profilabgleich nicht geladen werden:", err);
        }
    }
...
async function seedDocIfMissing(docRef, seedData) {
    try {
        ...
        console.warn("Firebase Seed-Dokument konnte nicht angelegt werden:", err);
    }
}
...
async function initFirebase() {
    ...
    try {
        ...
        usersDoc.onSnapshot({ includeMetadataChanges: true }, (snap) => {
            ...
            console.error("Firebase Benutzer-Synchronisierung fehlgeschlagen:", err);
            setSystemNotice(
                "error",
                "Benutzer-Synchronisierung blockiert",
                "Firebase konnte data/users nicht lesen. Bitte Firestore-Regeln und Firebase-Konfiguration pruefen."
            );
        });
...
async function uploadProfilePicToWorker(resized, username) {
    if (!resized || !resized.dataUrl) throw new Error("Kein komprimiertes Bild vorhanden");
    const res = await fetch(`${WT_WORKER_BASE}/api/profile-image/upload`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            username: username || currentUser || "user",
            filename: `profile-${Date.now()}.${resized.mimeType === "image/webp" ? "webp" : "jpg"}`,
            contentType: resized.mimeType || "image/jpeg",
            base64: dataUrlToBase64(resized.dataUrl)
        })
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || !data.url) {
        throw new Error(data.error || data.details || `Worker Upload fehlgeschlagen (${res.status})`);
    }
    return data.url;
}

async function resolveProfilePicUrlFromFile(file, username) {
    const resized = await resizeImageFile(file, { maxSize: 512, quality: 0.82, preferWebp: true });
    const uploadFile = new File(
        [resized.blob],
        'profile.' + (resized.mimeType === 'image/webp' ? 'webp' : 'jpg'),
        { type: resized.mimeType }
    );

    if (isFirebaseConnected && firebaseStorage) {
        try {
            return await uploadProfilePicToStorage(uploadFile, username);
        } catch (err) {
            console.warn("Firebase Storage Profilbild-Upload fehlgeschlagen, versuche Worker-Fallback:", err);
        }
    }

    try {
        return await uploadProfilePicToWorker(resized, username);
    } catch (err) {
        console.warn("Worker Profilbild-Upload fehlgeschlagen, verwende lokalen Data-URL-Fallback:", err);
    }

    if (resized.dataUrl) return resized.dataUrl;
    throw new Error("Profilbild konnte nicht vorbereitet werden.");
}
...
function getUserAvatar(username, sizeClasses = 'w-8 h-8', iconSize = 'w-4 h-4', clickable = false) {
    ...
    if (clickable && user && user.profilePicUrl && user.profilePicUrl.trim() !== '') {
        clickAttr = `onclick="showImageModal('${wtEscapeHtml(wtEscapeJsString(user.profilePicUrl))}')"`;
    ...
    if (user && user.profilePicUrl && user.profilePicUrl.trim() !== '') {
        ...
        return `<img src="${wtEscapeHtml(user.profilePicUrl)}" alt="Avatar" class="${wtEscapeHtml(sizeClasses)} ${cursorClass} rounded-full object-cover border border-gray-200 shadow-sm shrink-0" ${clickAttr} onerror="this.outerHTML='${safeAvatarHtml}'" />`;
    }
...
function renderApp() {
    ...
    if (renderNow - wtLastRenderStartedAt < 80) {
        if (!wtQueuedRenderTimer) {
            wtQueuedRenderTimer = setTimeout(() => {
                wtQueuedRenderTimer = null;
                renderApp();
            }, 90);
        }
        return;
    }
    wtLastRenderStartedAt = renderNow;
    ...
    if (!hasPassedGatekeeper) {
        const gatekeeperHtml = renderGatekeeper();
        const gatekeeperApp = document.getElementById('app');
        if (gatekeeperApp && wtLastRenderedAppHtml !== gatekeeperHtml) {
            gatekeeperApp.innerHTML = gatekeeperHtml;
            wtLastRenderedAppHtml = gatekeeperHtml;
        }
        restoreFocus();
        return;
    }
    ...
    if (!appEl) return;
    if (wtLastRenderedAppHtml === nextAppHtml) {
        restoreAuthModalDraft();
        restoreFocus();
        return;
    }
    wtLastRenderedAppHtml = nextAppHtml;
    appEl.innerHTML = nextAppHtml;
...
window.requestPasswordResetFromLogin = async function() {
    ...
    try {
        ...
        showModal("E-Mail gesendet", `Wir haben einen Link zum Zuruecksetzen an ${wtEscapeHtml(mappedEmail)} gesendet. Bitte pruefe auch deinen Spam- und Junk-Ordner.`);
    } catch (err) {
...
window.saveProfile = function() {
    ...
    if (user) {
        ...
        const applySave = () => {
            const save = (typeof window.saveUsersNow === "function")
                ? window.saveUsersNow()
                : Promise.resolve(window.saveState());
            Promise.resolve(save)
                .then(() => showModal('Erfolgreich', 'Dein Profil wurde gespeichert!'))
                .catch(err => {
                    console.error("Profil konnte nicht sofort gespeichert werden:", err);
                    window.saveState();
                    showModal('Erfolgreich', 'Dein Profil wurde lokal gespeichert und wird synchronisiert.');
                });
        };
        ...
        if (profilePicFile) {
            (async () => {
                try {
                    showModal('Upload laeuft...', 'Dein Profilbild wird hochgeladen.');
                    user.profilePicUrl = await resolveProfilePicUrlFromFile(profilePicFile, currentUser);
                    applySave();
                } catch (e) {
                    console.error('Profilbild Upload fehlgeschlagen:', e);
                    showModal('Fehler', 'Profilbild konnte nicht verarbeitet oder gespeichert werden.');
                }
            })();
            return;
...
window.clearProfilePic = function() {
    ...
    if(user) {
        ...
        Promise.resolve(save)
            .then(() => renderApp())
            .catch(err => {
                console.error("Profilbild konnte nicht geloescht werden:", err);
                window.saveState();
                renderApp();
            });
    }
...
/* =========================================================
   Artikel wirklich löschen Fix
   - Löschen gewinnt gegen Backup/Firebase/Last-Save-Fallback
   ========================================================= */
(function wtArticleHardDeleteFix() {
    if (window.__wtArticleHardDeleteFix) return;
    window.__wtArticleHardDeleteFix = true;

    const BACKUP_KEYS = [
        "wt_articles_backup_v2",
        "wt_articles_backup_v1",
        "wt_article_backup_v2",
        "wt_articles_admin_backup"
    ];
    const LAST_SAVE_KEY = "wt_articles_admin_last_save";
    const DELETED_KEY_EXTRA = "wt_article_deleted_keys_extra_v1";
    ...
    function articleKeys(article) {
        ...
        const title = normTitle(article.title);
        if (id) {
            keys.push("id:" + id);
            keys.push(id);
        }
    ...
    function readExtraDeletedKeys() {
        const arr = readJson(DELETED_KEY_EXTRA, []);
        return Array.isArray(arr) ? arr.map(String).filter(Boolean) : [];
    }

    function writeExtraDeletedKeys(keys) {
        writeJson(DELETED_KEY_EXTRA, Array.from(new Set((keys || []).map(String).filter(Boolean))));
    }

    function getDeletedSet() {
        let base = [];
        try {
            if (typeof getArticleDeletedKeys === "function") base = getArticleDeletedKeys();
        } catch (_) {}
        return new Set([...base, ...readExtraDeletedKeys()].map(String));
    }

    function rememberKeys(keys) {
        const merged = Array.from(new Set([...Array.from(getDeletedSet()), ...(keys || []).map(String)]));
        writeExtraDeletedKeys(merged);
        try {
            if (typeof setArticleDeletedKeys === "function") setArticleDeletedKeys(merged);
        } catch (_) {}
        return merged;
    }

    function articleIsDeleted(article) {
        const deleted = getDeletedSet();
        return articleKeys(article).some(k => deleted.has(k));
    }

    function filterDeletedFromList(list) {
        return (Array.isArray(list) ? list : []).filter(article => article && !articleIsDeleted(article));
    }

    function cleanAllArticleBackups(deletedArticle) {
        const deletedKeys = articleKeys(deletedArticle);
        rememberKeys(deletedKeys);

        BACKUP_KEYS.forEach(key => {
            const list = readJson(key, null);
            if (Array.isArray(list)) writeJson(key, filterDeletedFromList(list));
        });

        const last = readJson(LAST_SAVE_KEY, null);
        if (last && last.article && articleIsDeleted(last.article)) {
            try { localStorage.removeItem(LAST_SAVE_KEY); } catch (_) {}
        }
    }

    window.wtArticleKeysForDelete = articleKeys;
    window.wtArticleIsDeletedHard = articleIsDeleted;
    window.wtFilterDeletedArticlesHard = filterDeletedFromList;

    const oldRemember = window.wtRememberArticleDeleted;
    window.wtRememberArticleDeleted = function(article) {
        cleanAllArticleBackups(article);

        try {
            if (typeof oldRemember === "function") oldRemember(article);
        } catch (_) {}

        // oldRemember speichert oft nur id:. Wir speichern zusätzlich title:, damit andere Kopien nicht zurückkommen.
        rememberKeys(articleKeys(article));
        cleanAllArticleBackups(article);
        return true;
    };

    const oldPersist = window.wtPersistArticlesSafely;
    window.wtPersistArticlesSafely = async function(candidateArticles) {
        const cleanCandidate = filterDeletedFromList(candidateArticles);
        let result;
        if (typeof oldPersist === "function") {
            result = await oldPersist(cleanCandidate);
        } else if (typeof firebaseDb !== "undefined" && firebaseDb && typeof firebase !== "undefined") {
            await firebaseDb.collection("data").doc("articles").set({
                articles: cleanCandidate,
                deletedArticleKeys: Array.from(getDeletedSet()),
                updatedAt: firebase.firestore.FieldValue.serverTimestamp()
            }, { merge: true });
            result = cleanCandidate;
        } else {
            result = cleanCandidate;
        }

        const cleanResult = filterDeletedFromList(Array.isArray(result) ? result : cleanCandidate);

        try {
            if (Array.isArray(articles)) articles.splice(0, articles.length, ...cleanResult);
            else articles = cleanResult;
        } catch (_) {}
        window.articles = cleanResult;
        ...
        try {
            if (typeof firebaseDb !== "undefined" && firebaseDb && typeof firebase !== "undefined") {
                await firebaseDb.collection("data").doc("articles").set({
                    articles: cleanResult,
                    deletedArticleKeys: Array.from(getDeletedSet()),
                    updatedAt: firebase.firestore.FieldValue.serverTimestamp()
                }, { merge: true });
            }
        } catch (err) {
            console.warn("Finales Artikel-Löschen konnte nicht direkt nach Firebase geschrieben werden:", err);
        }

        return cleanResult;
    };

    // Laufende Absicherung gegen alte Snapshots/Backups
    setInterval(() => {
        try {
            const current = Array.isArray(window.articles) ? window.articles : (Array.isArray(articles) ? articles : []);
            const clean = filterDeletedFromList(current);
            if (clean.length !== current.length) {
                if (Array.isArray(articles)) articles.splice(0, articles.length, ...clean);
                else articles = clean;
                window.articles = clean;
                if (typeof renderApp === "function") renderApp();
            }
            ...
            BACKUP_KEYS.forEach(key => {
                ...
                if (Array.isArray(list)) {
                    const filtered = filterDeletedFromList(list);
                    if (filtered.length !== list.length) writeJson(key, filtered);
                }
            });
        } catch (_) {}
    }, 1200);
...
/* =========================================================
   Admin-Button Link Fix
   ========================================================= */
(function wtAdminButtonLinkFix() {
    if (window.__wtAdminButtonLinkFix) return;
    window.__wtAdminButtonLinkFix = true;

    window.openAdminPanel = function() {
        window.location.href = "adminZentrale.html";
    };

    window.goToAdminPanel = window.openAdminPanel;
    window.openAdmin = window.openAdminPanel;

    document.addEventListener("click", function(event) {
        const el = event.target.closest && event.target.closest("a, button, [onclick]");
        if (!el) return;

        const text = (el.textContent || "").trim().toLowerCase();
        const onclick = (el.getAttribute("onclick") || "").toLowerCase();

        const looksLikeAdminButton =
            text === "admin" &&
            (
                onclick.includes("adminlogin") ||
                onclick.includes("view='admin'") ||
                onclick.includes('view="admin"') ||
                onclick.includes("admintab")
            );

        if (!looksLikeAdminButton) return;
