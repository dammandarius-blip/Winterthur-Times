...
let hasLoadedRemoteArticlesOnce = false;
let lastRemoteArticlesSignature = "";
const WT_ARTICLE_BACKUP_KEY = "wt_articles_last_good_backup_v2";
...
function normalizeArticleTitleForPersistGuard(title) {
    return String(title || '').trim().toLowerCase().replace(/\s+/g, ' ');
}

function getArticleListSignature(list) {
    return (Array.isArray(list) ? list : [])
        .map(article => `${article && article.id ? article.id : ''}:${normalizeArticleTitleForPersistGuard(article && article.title ? article.title : '')}`)
        .join('|');
}

function getKnownDefaultArticleTitles() {
    return new Set([
        'ki verändert arbeitswelt',
        'einigung beim klimagipfel',
        'neues aus der stadtverwaltung',
        'neue ausstellung im kunst museum',
        'fc winterthur gewinnt derby',
        'winterthur eröffnet neuen veloweg'
    ]);
}

function countKnownDefaultArticles(list) {
    const known = getKnownDefaultArticleTitles();
    return (Array.isArray(list) ? list : []).filter(article => known.has(normalizeArticleTitleForPersistGuard(article && article.title))).length;
}

function backupArticleList(list) {
    try {
        if (!Array.isArray(list) || list.length === 0) return;
        const payload = {
            savedAt: new Date().toISOString(),
            count: list.length,
            articles: JSON.parse(JSON.stringify(list))
        };
        localStorage.setItem(WT_ARTICLE_BACKUP_KEY, JSON.stringify(payload));
    } catch (err) {
        console.warn('Artikel-Backup konnte nicht gespeichert werden:', err);
    }
}

function readArticleBackup() {
    try {
        const raw = localStorage.getItem(WT_ARTICLE_BACKUP_KEY);
        if (!raw) return null;
...
function shouldBlockSuspiciousDefaultOverwrite(nextArticles) {
    const backup = readArticleBackup();
    if (!backup || !Array.isArray(backup.articles) || backup.articles.length < 3) return false;
    if (!Array.isArray(nextArticles)) return false;

    const nextTitles = new Set(nextArticles.map(article => normalizeArticleTitleForPersistGuard(article && article.title)));
    const backupTitles = backup.articles.map(article => normalizeArticleTitleForPersistGuard(article && article.title)).filter(Boolean);
    const lostBackupTitles = backupTitles.filter(title => !nextTitles.has(title));
    const defaultCount = countKnownDefaultArticles(nextArticles);

    // Schutz gegen das typische Problem: alte Standardartikel überschreiben echte Admin-Artikel.
    return defaultCount >= 2 && lostBackupTitles.length >= Math.max(2, Math.ceil(backupTitles.length * 0.5));
}

function maybeRestoreArticlesFromLocalBackup(incomingArticles) {
    if (!shouldBlockSuspiciousDefaultOverwrite(incomingArticles)) return null;
    const backup = readArticleBackup();
    if (!backup || !Array.isArray(backup.articles)) return null;
    console.warn('Verdächtiger Artikel-Reset erkannt. Lokales Artikel-Backup wird verwendet:', backup.savedAt);
    return JSON.parse(JSON.stringify(backup.articles));
}
...
async function persistRemoteState() {
    ...
    const blockArticleSave = shouldBlockSuspiciousDefaultOverwrite(articles);
    if (blockArticleSave) {
        console.error('Artikel-Speicherung blockiert: Die aktuelle Liste sieht wie ein alter Standardartikel-Reset aus. Firebase wird nicht mit Standardartikeln überschrieben.');
    } else {
        backupArticleList(articles);
        lastRemoteArticlesSignature = getArticleListSignature(articles);
...
function scheduleRemoteSave() {
    ...
    if (isApplyingRemoteState) return;
    if (!hasLoadedRemoteArticlesOnce) {
        console.warn('Speichern übersprungen: Firebase-Artikel wurden noch nicht geladen.');
        return;
    }
        ...
        articlesDoc.onSnapshot({ includeMetadataChanges: true }, (snap) => {
            ...
            try {
                if (Array.isArray(data.articles)) {
                    const restoredArticles = maybeRestoreArticlesFromLocalBackup(data.articles);
                    if (restoredArticles) {
                        articles = restoredArticles;
                        setTimeout(() => persistRemoteState().catch(err => console.error('Backup-Wiederherstellung fehlgeschlagen:', err)), 1200);
                    } else {
                        articles = data.articles;
                        hasLoadedRemoteArticlesOnce = true;
                        lastRemoteArticlesSignature = getArticleListSignature(articles);
                        backupArticleList(articles);
                    }
                }
