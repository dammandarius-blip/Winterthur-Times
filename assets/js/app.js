...
function applyRemoteArticlesDoc(data) {
    ...
    return { shouldRepairRemoteArticles, protectedArticles };
}
...
async function initFirebase() {
    ...
    try {
        ...
        try {
            const [initialArticlesSnap, initialChatsSnap, initialUsersSnap] = await Promise.all([
                articlesDoc.get(),
                chatsDoc.get(),
                usersDoc.get()
            ]);
            isApplyingRemoteState = true;
            try {
                if (initialArticlesSnap.exists) applyRemoteArticlesDoc(initialArticlesSnap.data() || {});
                if (initialChatsSnap.exists) {
                    const data = initialChatsSnap.data() || {};
                    if (Array.isArray(data.supportChats)) supportChats = data.supportChats;
                }
                applyRemoteUsersDoc(initialUsersSnap.exists ? (initialUsersSnap.data() || {}) : {});
            } finally {
                isApplyingRemoteState = false;
            }
        } catch (err) {
            console.warn("Firebase Anfangsdaten konnten nicht komplett vorgeladen werden:", err);
        }
        ...
        articlesDoc.onSnapshot({ includeMetadataChanges: true }, (snap) => {
            ...
            let result = { shouldRepairRemoteArticles: false, protectedArticles: null };
            isApplyingRemoteState = true;
            try {
                result = applyRemoteArticlesDoc(data);
            } finally {
                isApplyingRemoteState = false;
            }
            if (result.shouldRepairRemoteArticles && result.protectedArticles) {
                firebaseDb.collection("data").doc("articles").set({
                    articles: result.protectedArticles,
                    deletedArticleKeys: getArticleDeletedKeys(),
                    updatedAt: firebase.firestore.FieldValue.serverTimestamp()
...
function updateWeatherDisplay() {
    const textEl = document.getElementById("wt-weather-text");
    const iconEl = document.getElementById("wt-weather-icon");
    if (!wtHasPaintedAppOnce || !textEl || !iconEl) {
        return;
    }
    textEl.textContent = `${currentWeather.city}, ${currentWeather.temp}`;
    iconEl.setAttribute("data-lucide", currentWeather.icon);
            ...
            <div class="flex justify-end items-center gap-4 text-sm font-sans text-gray-600 min-w-0">
                <div class="hidden md:flex flex-col items-end">
                    ...
                    <div class="flex items-center gap-1" title="Aktuelles Wetter">
                        <i id="wt-weather-icon" data-lucide="${currentWeather.icon}" class="w-4 h-4"></i>
                        <span id="wt-weather-text">${currentWeather.city}, ${currentWeather.temp}</span>
                    </div>
                </div>
            </div>
        </div>
