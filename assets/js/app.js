...
async function saveUsersNow() {
    ...
    if (isFirebaseConnected && firebaseDb && typeof firebase !== "undefined") {
        await firebaseDb.collection("data").doc("users").set({
            registeredUsers: sanitizeUsersForRemote(registeredUsers),
...
/* =========================================================
   Tagesbilder Like-Persistenz Fix
   - Likes auf lokalen Fallback-Bildern bleiben erhalten
   - Likes werden nach Firebase-Refresh wieder gemerged
   ========================================================= */
(function wtGalleryLikePersistenceFix() {
    if (window.__wtGalleryLikePersistenceFix) return;
    window.__wtGalleryLikePersistenceFix = true;

    const LOCAL_LIKES_KEY = "wt_gallery_local_likes_v1";
    ...
    function writeJson(key, value) {
        try {
            ...
            console.warn("Lokale Tagesbilder-Daten konnten nicht gespeichert werden:", err);
        }
    }
    ...
    function getCurrentUserSafe() {
        ...
        try {
            const stored = localStorage.getItem("winterthur_user") || localStorage.getItem("currentUser") || "";
    ...
    function getImagesArray() {
        ...
        if (Array.isArray(window.communityImages)) return window.communityImages;
        return [];
    }

    function findImageById(id) {
        const idStr = String(id);
        return getImagesArray().find(img => String(img.id) === idStr) || null;
    }

    function getLocalLikesMap() {
        const map = readJson(LOCAL_LIKES_KEY, {});
        return map && typeof map === "object" ? map : {};
    }

    function saveLocalLikesMap(map) {
        writeJson(LOCAL_LIKES_KEY, map || {});
    }
    ...
    function mergeLikesForImage(img) {
        if (!img || !img.id) return img;

        const idStr = String(img.id);
        const map = getLocalLikesMap();
        const localLikes = normalizeLikes(map[idStr]);
        const remoteLikes = normalizeLikes(img.likes);
        const merged = Array.from(new Set([...remoteLikes, ...localLikes]));

        img.likes = merged;
        return img;
    }

    function mergeLikesIntoAllImages() {
        const arr = getImagesArray();
        arr.forEach(mergeLikesForImage);

        // Auch lokale Fallback-Bilder im localStorage aktualisieren
        const localImages = readJson(LOCAL_IMAGES_KEY, []);
        if (Array.isArray(localImages) && localImages.length) {
            localImages.forEach(mergeLikesForImage);
            writeJson(LOCAL_IMAGES_KEY, localImages);
        }
    }

    function persistImageLikes(img) {
        if (!img || !img.id) return;

        const map = getLocalLikesMap();
        const idStr = String(img.id);
        map[idStr] = normalizeLikes(img.likes);
        saveLocalLikesMap(map);

        const localImages = readJson(LOCAL_IMAGES_KEY, []);
        if (Array.isArray(localImages) && localImages.length) {
            const local = localImages.find(x => String(x.id) === idStr);
            if (local) {
                local.likes = normalizeLikes(img.likes);
                writeJson(LOCAL_IMAGES_KEY, localImages);
            }
        }
    }

    function removeExpiredLikeEntries() {
        const images = getImagesArray();
        const validIds = new Set(images.filter(img => !img.isDeleted).map(img => String(img.id)));

        const localImages = readJson(LOCAL_IMAGES_KEY, []);
        if (Array.isArray(localImages)) {
            localImages.filter(img => !img.isDeleted).forEach(img => validIds.add(String(img.id)));
        }
        ...
        if (changed) saveLocalLikesMap(map);
    }

    // Eigene Like-Funktion, damit Firebase-Refresh die Likes nicht sofort wieder überschreibt.
    window.toggleCommunityImageLike = function(id) {
        ...
        mergeLikesIntoAllImages();

        const idStr = String(id);
        const img = findImageById(idStr);
        if (!img) return;

        img.likes = normalizeLikes(img.likes);

        const idx = img.likes.map(String).indexOf(user);
        if (idx >= 0) {
        ...
        persistImageLikes(img);

        // Für lokale Fallback-Bilder ist localStorage der wichtigste Speicher.
        // Trotzdem versuchen wir Firebase zu speichern, falls es ein normales Bild ist.
        try {
            if (typeof window.saveState === "function") window.saveState();
        } catch (err) {
            console.warn("Like konnte nicht remote gespeichert werden, lokal bleibt er erhalten:", err);
        }
    ...
    window.toggleCommunityImageLike.__wtLikePersistencePatched = true;

    // renderApp patchen: vor jedem Render lokale Likes wieder hineinmischen.
    const originalRender = window.renderApp;
    if (typeof originalRender === "function" && !originalRender.__wtLikePersistencePatched) {
        const patchedRender = function(...args) {
            mergeLikesIntoAllImages();
            removeExpiredLikeEntries();
            return originalRender.apply(this, args);
        };
        patchedRender.__wtLikePersistencePatched = true;
        window.renderApp = patchedRender;
    }

    // Nach Firebase-Updates regelmässig lokale Likes zurück in den Speicher mischen.
    setInterval(() => {
        mergeLikesIntoAllImages();
        removeExpiredLikeEntries();
    }, 1000);
    ...
    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", mergeLikesIntoAllImages);
    } else {
        mergeLikesIntoAllImages();
    }
})();
...
/* =========================================================
   Tagesbilder Like FINAL Fix
   - Like bleibt rot
   - Anzahl bleibt erhalten
   - funktioniert auch bei Firebase-Refresh und lokalen Fallback-Bildern
   ========================================================= */
(function wtGalleryLikeFinalFix() {
    if (window.__wtGalleryLikeFinalFix) return;
    window.__wtGalleryLikeFinalFix = true;

    const LOCAL_LIKES_KEY = "wt_gallery_like_state_final_v1";
    const LOCAL_IMAGES_KEY = "wt_gallery_local_fallback_images_v1";
    ...
    function writeJson(key, value) {
        try {
            ...
            console.warn("Lokale Like-Daten konnten nicht gespeichert werden:", err);
        }
    }
    ...
    function userKey() {
        ...
        try {
            if (typeof loggedInUser !== "undefined" && loggedInUser) {
                ...
                return String(loggedInUser.username || loggedInUser.name || loggedInUser.email || loggedInUser.id || "user");
            }
        } catch (_) {}
        ...
        // Fallback: damit Likes auch ohne korrekt erkannte Variable lokal stabil bleiben
        let anon = localStorage.getItem("wt_gallery_anon_like_user");
        if (!anon) {
            anon = "anon_" + Math.random().toString(36).slice(2) + Date.now();
            localStorage.setItem("wt_gallery_anon_like_user", anon);
        }
        return anon;
    }
    ...
    function imageArrays() {
        const arrays = [];
        try {
            if (typeof communityImages !== "undefined" && Array.isArray(communityImages)) arrays.push(communityImages);
        } catch (_) {}
        if (Array.isArray(window.communityImages) && !arrays.includes(window.communityImages)) arrays.push(window.communityImages);
        return arrays;
    }

    function localLikesMap() {
        const map = readJson(LOCAL_LIKES_KEY, {});
        return map && typeof map === "object" ? map : {};
    }
    ...
    function findImage(id) {
        ...
        for (const arr of imageArrays()) {
            const found = arr.find(img => String(img && img.id) === idStr);
            if (found) return found;
        }
        const local = readJson(LOCAL_IMAGES_KEY, []);
        if (Array.isArray(local)) {
            const found = local.find(img => String(img && img.id) === idStr);
            if (found) return found;
        }
        return null;
    }

    function saveImageIntoLocalFallbackIfExists(img) {
        if (!img || !img.id) return;
        const idStr = String(img.id);
        const local = readJson(LOCAL_IMAGES_KEY, []);
        if (!Array.isArray(local) || !local.length) return;

        const idx = local.findIndex(x => String(x && x.id) === idStr);
        if (idx >= 0) {
            local[idx].likes = normalizeLikes(img.likes);
            writeJson(LOCAL_IMAGES_KEY, local);
        }
    }

    function applyLocalLikesToImage(img) {
        if (!img || !img.id) return img;

        const map = localLikesMap();
        const idStr = String(img.id);
        const localLikes = normalizeLikes(map[idStr]);
        const remoteLikes = normalizeLikes(img.likes);
        img.likes = Array.from(new Set([...remoteLikes, ...localLikes]));
        return img;
    }

    function applyLocalLikesEverywhere() {
        imageArrays().forEach(arr => arr.forEach(applyLocalLikesToImage));

        const local = readJson(LOCAL_IMAGES_KEY, []);
        if (Array.isArray(local) && local.length) {
            local.forEach(applyLocalLikesToImage);
            writeJson(LOCAL_IMAGES_KEY, local);
        }
    }

    function persistLikesForImage(img) {
        if (!img || !img.id) return;
        const map = localLikesMap();
        map[String(img.id)] = normalizeLikes(img.likes);
        saveLocalLikesMap(map);
        saveImageIntoLocalFallbackIfExists(img);
    }

    function setLikesOnAllCopies(id, likes) {
        const idStr = String(id);
        const normalized = normalizeLikes(likes);

        imageArrays().forEach(arr => {
            arr.forEach(img => {
                if (String(img && img.id) === idStr) img.likes = normalized.slice();
            });
        });
    ...
    function syncLocalFallbackImagesIntoMemory() {
        ...
        valid.forEach(applyLocalLikesToImage);

        imageArrays().forEach(arr => {
            const ids = new Set(arr.map(img => String(img && img.id)));
            valid.slice().reverse().forEach(img => {
                if (!ids.has(String(img.id))) arr.unshift(img);
            });
        });
    }

    function doToggleLike(id) {
        const idStr = String(id);
        const user = userKey();

        syncLocalFallbackImagesIntoMemory();
        applyLocalLikesEverywhere();

        let img = findImage(idStr);
        if (!img) return;

        img.likes = normalizeLikes(img.likes);
        const idx = img.likes.indexOf(user);

        if (idx >= 0) img.likes.splice(idx, 1);
        else img.likes.push(user);

        img.likes = normalizeLikes(img.likes);
        setLikesOnAllCopies(idStr, img.likes);
        persistLikesForImage(img);

        try {
            if (typeof window.saveState === "function") {
                // Nicht awaiten: lokaler Zustand soll sofort bleiben, auch wenn Firebase kurz alten Stand zurücksendet.
                window.saveState();
            }
        } catch (err) {
            console.warn("Like remote speichern fehlgeschlagen, lokal bleibt er erhalten:", err);
        }
    ...
    // Wichtig: beide Namen patchen, falls die Seite einen anderen Namen nutzt.
    window.toggleCommunityImageLike = doToggleLike;
    window.likeCommunityImage = doToggleLike;
    window.toggleGalleryImageLike = doToggleLike;

    // Falls im HTML bereits onclick="toggleCommunityImageLike(local_...)" ohne Quotes steht,
    // fangen wir den Klick zusätzlich per Event Delegation ab.
    document.addEventListener("click", function(event) {
        const btn = event.target.closest && event.target.closest("[onclick*='toggleCommunityImageLike'], [onclick*='likeCommunityImage'], [data-gallery-like-id]");
        if (!btn) return;

        const dataId = btn.dataset.galleryLikeId || btn.getAttribute("data-id") || btn.getAttribute("data-image-id");
        if (dataId) {
            event.preventDefault();
            event.stopPropagation();
            event.stopImmediatePropagation();
    ...
    if (typeof oldRender === "function" && !oldRender.__wtGalleryLikeFinalPatched) {
        const patchedRender = function(...args) {
            syncLocalFallbackImagesIntoMemory();
            applyLocalLikesEverywhere();
            return oldRender.apply(this, args);
        };
        patchedRender.__wtGalleryLikeFinalPatched = true;
        window.renderApp = patchedRender;
    }

    // Nach jedem Firebase-Snapshot alten Stand wieder mit lokalen Likes reparieren.
    setInterval(() => {
        syncLocalFallbackImagesIntoMemory();
        applyLocalLikesEverywhere();
    }, 400);
