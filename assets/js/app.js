    ...
    <header class="border-b border-gray-300 sticky top-0 bg-[#fcfbf9] z-40">
        <div class="max-w-7xl mx-auto px-3 sm:px-4 py-2 sm:py-4 grid grid-cols-[auto_1fr_auto] items-center gap-2 sm:gap-4">
            ...
            <div class="min-w-0 text-center">
                ...
                <!-- Professionelles Tagline unterhalb des Logos -->
                <p class="text-[0.6rem] sm:text-xs md:text-sm font-sans font-medium tracking-wider uppercase text-gray-500 mt-0.5">
...
window.wtGetPublicSupportContext = function() {
    ...
    const loggedInState = currentUser ? "eingeloggt" : "nicht eingeloggt";

    const viewNames = {
        home: "Startseite",
        article: "Artikelansicht",
        gallery: "Tagesbilder / Community-Galerie",
        games: "Spiele-Seite",
        authors: "Unsere Autoren",
        feedback: "Website bewerten / Feedback",
        profile: "Profilbereich",
        "admin-login": "nicht öffentliche Admin-Anmeldung"
    };

    return [
        `Aktueller öffentlicher Bereich: ${viewNames[currentView] || currentView}${currentCategory ? " / Ressort " + currentCategory : ""}.`,
        `Nutzerstatus: ${loggedInState}.`,
        `Aktuelle Ressorts/Kategorien: ${activeCategories.join(", ") || "keine"}.`,
        "Professioneller Header: schwarze Top-Leiste oben, darunter weißer Kopfbereich mit Logo links, Menü-Icon, Such-Icon, großem Titel Winterthur Times, Tagline Unabhängig. Regional. Aktuell. sowie Datum/Wetter rechts.",
        "Browser-Tab/Favicon: Das Website-Logo liegt als assets/logo.png vor und ist in index.html per link rel=icon eingebunden.",
        "Hauptnavigation: Menü-Icon oben links öffnet das Seitenmenü. Lupe oben links öffnet die Suchleiste direkt unter dem Header. Titel und Logo führen zur Startseite.",
        "Startseite: Hauptartikel links, weitere Artikelkarten darunter, rechte Spalte mit Meistgelesen, Tägliches Rätsel, Tagesbilder, Sudoku und Kreuzworträtsel.",
        "Tagesbilder: erreichbar über Top-Leiste, Menü, grünes Widget Zur Galerie und Footer-Link Tagesbilder (Community). Eingeloggte Nutzer veröffentlichen Bilder über Bild veröffentlichen.",
        "Spiele: erreichbar über Menü/Ressort Spiele. Enthält Solitaire, Sudoku, Kreuzworträtsel und Tägliches Rätsel. Solitaire hat die Buttons Tipps, Zurück und Neu starten.",
        "Footer: schwarze Fußzeile mit Winterthur Times, Ressorts, Service und Verlag. Öffentlicher Support soll keine Admin-Anleitungen ausgeben.",
        "Support-Chat: runder blauer Button unten rechts, Eingabefeld Wie können wir helfen?... und Senden-Button mit Papierflieger. KI-Schalter im Chatfenster."
    ].join("\n");
};
