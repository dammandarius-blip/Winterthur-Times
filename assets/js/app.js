...
function wtBreakingFirstSort(list) {
    const arr = Array.isArray(list) ? list.slice() : [];
    return arr.sort((a, b) => {
        const ab = a && (a.isEilmeldung === true || a.isBreaking === true || a.breaking === true) ? 1 : 0;
        const bb = b && (b.isEilmeldung === true || b.isBreaking === true || b.breaking === true) ? 1 : 0;
...
function renderHome() {
    ...
    const now = new Date();
    const currentArticles = wtBreakingFirstSort(articles);
...
function renderSearchResults() {
    ...
    if (searchCategory) {
        results = wtBreakingFirstSort(currentArticles.filter(a => a.category === searchCategory));
        titleHtml = `Ressort: ${searchCategory}`;
    } else {
        const query = searchQuery;
        results = wtBreakingFirstSort(currentArticles.filter(a => wtArticleMatchesSearch(a, query)));
...
/* Eilmeldung Runtime-Sortierung */
(function wtBreakingRuntimeSort() {
    if (window.__wtBreakingRuntimeSort) return;
    window.__wtBreakingRuntimeSort = true;

    function sortNow() {
        try {
            if (typeof articles !== "undefined" && Array.isArray(articles) && typeof wtBreakingFirstSort === "function") {
                const sorted = wtBreakingFirstSort(articles);
                articles.splice(0, articles.length, ...sorted);
            }
        } catch (_) {}
        if (Array.isArray(window.articles) && typeof wtBreakingFirstSort === "function") {
            const sorted = wtBreakingFirstSort(window.articles);
            window.articles.splice(0, window.articles.length, ...sorted);
        }
    }

    sortNow();
    setInterval(sortNow, 2000);
})();
