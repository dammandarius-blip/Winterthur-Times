...
async function uploadCommunityImageToStorage(file, username) {
    if (!isFirebaseConnected || !firebaseStorage) throw new Error('Firebase Storage nicht verfuegbar');
    if (!file) throw new Error('Keine Datei');
    if (file.type && !file.type.startsWith('image/')) throw new Error('Bitte eine Bilddatei auswaehlen.');

    const safeName = (username || 'user').toString().replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 64);
    const originalExt = (file.name || '').split('.').pop();
    const ext = (originalExt && originalExt.length <= 5)
        ? (originalExt.toLowerCase().replace(/[^a-z0-9]/g, '') || 'jpg')
        : (file.type === 'image/png' ? 'png' : file.type === 'image/webp' ? 'webp' : 'jpg');
    const path = `community_images/${safeName}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}.${ext}`;
    const ref = firebaseStorage.ref().child(path);
...
window.handleCommunityUpload = async function() {
    ...
    const saveImg = (src, meta = {}) => {
        ...
        communityImages.push({
            ...
            isDeleted: false,
            likes: [],
            storageBacked: !!meta.storageBacked,
    ...
    if (fileInput && fileInput.files && fileInput.files[0]) {
        ...
        try {
            if (isFirebaseConnected && firebaseStorage) {
                showModal('Upload laeuft', 'Dein Tagesbild wird hochgeladen. Grosse Bilder werden jetzt als Datei gespeichert, nicht mehr direkt in der Datenbank.');
                const url = await uploadCommunityImageToStorage(file, currentUser);
                saveImg(url, { storageBacked: true, originalName: file.name || "", originalSize: file.size || 0 });
            } else {
                const resized = await resizeImageFile(file, { maxSize: 1800, quality: 0.78, preferWebp: true });
                if (resized.dataUrl) {
                    saveImg(resized.dataUrl, { originalName: file.name || "", originalSize: file.size || 0 });
                } else {
                    const reader = new FileReader();
                    reader.onload = function(e) { saveImg(e.target.result, { originalName: file.name || "", originalSize: file.size || 0 }); };
                    reader.readAsDataURL(file);
                }
            }
        } catch (error) {
            console.error('Tagesbild Upload fehlgeschlagen:', error);
            showModal('Fehler', 'Das Tagesbild konnte nicht hochgeladen werden. Pruefe Firebase Storage und die Upload-Regeln.');
        }
