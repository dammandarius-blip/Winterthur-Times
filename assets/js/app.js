    ...
    try {
        ...
        articlesDoc.onSnapshot({ includeMetadataChanges: true }, (snap) => {
            ...
            try {
                ...
                if (Array.isArray(data.communityImages)) communityImages = data.communityImages.filter(img => {
                    try {
                        const deleted = JSON.parse(localStorage.getItem("wt_gallery_deleted_ids_v1") || "[]").map(String);
                        return img && !img.isDeleted && !deleted.includes(String(img.id));
                    } catch (_) {
                        return img && !img.isDeleted;
                    }
                });
