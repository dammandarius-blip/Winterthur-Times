        ...
        <div class="max-w-7xl mx-auto px-3 sm:px-4 py-2 sm:py-4 grid grid-cols-[auto_1fr_auto] items-center gap-2 sm:gap-4">
            ...
            <div class="min-w-0 text-center">
                <h1 onclick="setView('home')"
                    class="flex flex-col items-center justify-center text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-tight uppercase font-serif cursor-pointer hover:text-blue-900 transition-colors leading-none">
                    <!-- Logo and Title -->
                    <span class="flex items-center justify-center gap-2">
                        <img src="./assets/img/logo.svg" alt="Winterthur Times" class="h-6 w-6 sm:h-8 sm:w-8 md:h-10 md:w-10">
                        <span>Winterthur Times</span>
                    </span>
                    <!-- Tagline below the title -->
                    <span class="mt-1 text-xs sm:text-sm md:text-base font-normal tracking-wider text-gray-600 uppercase">
                        Unabhängig. Regional. Aktuell.
                    </span>
                </h1>
            </div>
...
function getArticleImageUrlKey(value) {
    ...
    try {
        ...
        // Entferne Hash und Query‑Parameter, damit unterschiedliche Anhang‑Strings nicht zu Duplikaten führen
        parsed.hash = '';
        parsed.search = '';
...
function dedupeArticleInlineImages(article) {
    ...
    images.forEach(image => {
        ...
        if (!urlKey || !Number.isFinite(position) || position < 1 || seenUrls.has(urlKey)) return;

        seenUrls.add(urlKey);
        cleaned.push({
            ...image,
            url,
            positionAfterParagraph: Math.floor(position)
        });
    });
...
function wtArticleMatchesSearch(article, query) {
    ...
    // Erweitere die Suchbegriffe (Synonyme, Grundformen)
    let terms = wtExpandedSearchTerms(query);
    // Wenn keine Synonyme oder Stämme gefunden wurden, verwende den normalisierten Query selbst,
    // damit eine direkte Wortsuche stattfindet. Dadurch wird verhindert, dass alle Artikel
    // zurückgegeben werden, wenn der Benutzer einen unbekannten Begriff eingibt.
    if (!terms || !terms.length) {
        const norm = wtNormalizeSearch(String(query));
        if (norm) terms = [norm];
        else return false;
    }
...
// Globale Einstellung für die Quelle des "Über&nbsp;uns"‑Textes.
// Standardmäßig wird die lokale Datei ./assets/about.txt geladen. Optional kann
// "window.WT_ABOUT_URL" vom Worker oder einer anderen Konfiguration gesetzt
// werden, um den Text aus einem GitHub‑Raw‑Link zu laden. Beispiel:
// window.WT_ABOUT_URL = 'https://raw.githubusercontent.com/<owner>/<repo>/main/about.txt';
const DEFAULT_ABOUT_TXT = './assets/about.txt';

// Fallback‑Text für "Über uns", falls das Laden der externen Datei fehlschlägt (z. B. im lokalen
// Dateisystem aufgrund von Content‑Security‑Policy oder fehlender Netzwerkverbindung). Dieser Text
// wird verwendet, wenn der Abruf aus GitHub oder aus der lokalen Datei nicht gelingt.
const FALLBACK_ABOUT_TEXT = `Die Winterthur Times ist eine unabhängige Nachrichtenplattform für Winterthur
und die Region. Hier findest du aktuelle Berichte, Hintergründe und Meinungen
aus Politik, Wirtschaft, Gesellschaft, Kultur, Sport und Wissenschaft. Unsere
Redaktion besteht aus engagierten Schülerinnen und Schülern, die journalistische
Erfahrung sammeln und ihre Leidenschaft für die Medienwelt ausleben möchten.`;

function sanitizeAboutText(value) {
    // Entfernt problematische HTML‑Zeichen und ersetzt Zeilenumbrüche durch <br>
    const escaped = escapeNavText(value);
    return escaped.replace(/\n/g, '<br>');
}

window.showAbout = function() {
    const url = (typeof window.WT_ABOUT_URL === 'string' && window.WT_ABOUT_URL.trim()) ? window.WT_ABOUT_URL.trim() : DEFAULT_ABOUT_TXT;
    ...
    fetch(fetchUrl)
        ...
        .then(text => {
            const safeText = sanitizeAboutText(text);
            showModal('Über uns', safeText);
        })
        .catch(err => {
            console.error('Konnte Über-uns-Text nicht laden:', err);
            // Fallback: Eingebetteten Text anzeigen, sofern definiert
            const safeText = sanitizeAboutText(FALLBACK_ABOUT_TEXT);
            showModal('Über uns', safeText);
        });
};
        ...
        <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
                ...
                <ul class="flex flex-col gap-2 text-sm text-gray-400">
                    ...
                    <li><span onclick="showAbout(); window.scrollTo(0,0);" class="cursor-pointer hover:text-white transition-colors">Über uns</span></li>
            ...
            ${currentModal.type === 'login' ? `
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <div class="text-center text-sm text-gray-600">
                        Passwort vergessen? <button onclick="forgotPassword()" class="text-blue-700 font-bold hover:underline cursor-pointer">Zurücksetzen</button>
                    </div>
...
// Versendet einen Link zum Zurücksetzen des Passworts an die angegebene E-Mail-Adresse.
// Wenn Firebase verbunden ist, wird die integrierte Funktion verwendet. Bei rein lokaler
// Nutzung wird ein Fallback über den Cloudflare Worker versucht. Für externe
// Sicherheit wird kein Klartext-Passwort verschickt.
window.forgotPassword = async function() {
    try {
        const usernameInput = document.getElementById('usernameInput');
        const identifier = usernameInput ? usernameInput.value.trim() : '';
        if (!identifier) {
            showWarning('Bitte gib deinen Benutzernamen oder deine E-Mail ein, um dein Passwort zurückzusetzen.');
            return;
        }
        ...
        if (identifier.includes('@')) {
            ...
            const existing = registeredUsers.find(u => u.username === identifier);
            email = existing && existing.email ? existing.email.trim() : '';
        }

        if (!email || !email.includes('@')) {
            showWarning('Es konnte keine gültige E-Mail-Adresse gefunden werden. Bitte gib deine registrierte E-Mail ein.');
            return;
        }

        const resetButton = document.querySelector('button[onclick="forgotPassword()"]');
        if (resetButton) {
            resetButton.disabled = true;
            resetButton.textContent = 'Sende...';
        }

        // 1) Neuer Standardweg: Cloudflare Worker.
        // Vorteil: Der Firebase API Key und spätere Mail-Logik bleiben sauber im Backend.
        try {
            const res = await fetch(`${WT_WORKER_BASE}/api/forgot-password`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (res.ok && data && data.ok) {
                showModal(
                    ...
                    `Wenn ${email} bei uns registriert ist, wurde ein Link zum Zurücksetzen des Passworts gesendet. Bitte prüfe auch den Spam-Ordner.`
                );
                return;
            }

            console.warn('Worker Passwort-Reset nicht erfolgreich:', data);
        } catch (workerErr) {
            console.warn('Worker Passwort-Reset nicht erreichbar, versuche Firebase direkt:', workerErr);
        }

        // 2) Fallback: Firebase direkt im Browser, falls Worker noch nicht deployed/konfiguriert ist.
        if (isFirebaseConnected && firebaseAuth) {
            try {
                ...
                console.error('Firebase Passwort-Reset Fehler:', firebaseErr);
                const code = firebaseErr && firebaseErr.code ? firebaseErr.code : '';

                if (code === 'auth/user-not-found') {
                    showWarning('Diese E-Mail ist noch nicht als Account registriert. Erstelle zuerst einen Account.');
                } else if (code === 'auth/unauthorized-domain') {
                    showWarning('Diese Website-Domain ist in Firebase noch nicht autorisiert.');
                } else if (code === 'auth/invalid-api-key') {
                    showWarning('Der Firebase API-Key ist falsch oder fehlt.');
                } else if (code === 'auth/too-many-requests') {
                    showWarning('Zu viele Versuche. Bitte warte einige Minuten und probiere es erneut.');
                } else {
                    showWarning(`Der Passwort-Reset konnte nicht gesendet werden. Firebase-Fehler: ${code || 'unbekannt'}`);
                }
                return;
            }
        }

        showWarning('Der Passwort-Reset konnte nicht gesendet werden, weil weder Worker noch Firebase erreichbar sind.');
    } catch (err) {
        console.error('Passwort-Reset-Fehler:', err);
        showWarning('Der Passwort-Reset konnte nicht gesendet werden. Bitte versuche es später erneut.');
    } finally {
        const resetButton = document.querySelector('button[onclick="forgotPassword()"]');
        if (resetButton) {
