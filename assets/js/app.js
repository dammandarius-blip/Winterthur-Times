...
function wtSolitaireNewGame() {
    ...
    let deck = [];
    suits.forEach(suit => ranks.forEach((rank, i) => deck.push({ suit, rank, value: i + 1, color: colors[suit], faceUp: false })));
    ...
    window.wtSolitaire = { stock: deck, waste: [], foundations: { "♠": [], "♥": [], "♦": [], "♣": [] }, tableau, moves: 0, selected: null, history: [] };
    wtSolitaireSave();
}

function wtSolitaireSave() {
    try { localStorage.setItem("wt_solitaire_games_page", JSON.stringify(window.wtSolitaire)); } catch (_) {}
}

function wtSolitaireLoad() {
    if (window.wtSolitaire) return;
    try {
        const saved = JSON.parse(localStorage.getItem("wt_solitaire_games_page") || "null");
        if (saved && Array.isArray(saved.tableau)) window.wtSolitaire = saved;
    } catch (_) {}
    if (!window.wtSolitaire) wtSolitaireNewGame();
}

function wtSolCardHtml(card, attrs = "", selected = false) {
    if (!card) return `<button type="button" class="wt-sol-card empty" ${attrs}></button>`;
    if (!card.faceUp) return `<button type="button" class="wt-sol-card back" ${attrs}>WT</button>`;
    return `<button type="button" class="wt-sol-card ${card.color} ${selected ? 'selected' : ''}" ${attrs}><span>${card.rank}</span><strong>${card.suit}</strong></button>`;
}
...
function renderSimpleSolitaireGame() {
    ...
    const foundations = ["♠", "♥", "♦", "♣"].map(suit => {
        const top = g.foundations[suit][g.foundations[suit].length - 1];
        return top ? wtSolCardHtml(top, `data-wt-foundation="${suit}"`) : `<button type="button" class="wt-sol-card empty" data-wt-foundation="${suit}">${suit}</button>`;
    }).join("");
    const columns = g.tableau.map((col, c) => `<div class="wt-sol-column" data-wt-sol-column="${c}">
        ${col.map((card, i) => `<div class="wt-sol-pos" style="top:${i * 26}px">${wtSolCardHtml(card, `data-wt-sol-card="${c}" data-wt-sol-index="${i}"`, g.selected && g.selected.type === 'tableau' && g.selected.col === c && g.selected.index === i)}</div>`).join("") || `<button type="button" class="wt-sol-card empty" data-wt-sol-column="${c}">K</button>`}
    </div>`).join("");
    return `
    <section class="wt-solitaire-game">
        <div class="wt-sol-head">
            <div><h2>Solitaire</h2><p>Klicke eine Karte an und danach den Zielstapel. Doppelklick legt passende Karten automatisch ab.</p></div>
            <div class="wt-sol-actions">
                <button onclick="wtSolitaireHint()">Tipps</button>
                <button onclick="wtSolitaireUndo()">Zurück</button>
                <button onclick="wtSolitaireNewGame(); renderApp();">Neu starten</button>
            </div>
        </div>
        ...
        <p id="wtSolitaireHint" class="wt-sol-hint">Tipp: Asse nach oben legen. Auf den unteren Stapeln müssen die Farben abwechseln.</p>
    </section>`;
}
...
function wtSolitaireSnapshot() {
    const g = window.wtSolitaire;
    g.history = g.history || [];
    g.history.push(JSON.stringify({ stock: g.stock, waste: g.waste, foundations: g.foundations, tableau: g.tableau, moves: g.moves }));
...
function wtSolitaireUndo() {
    ...
    window.wtSolitaire = { ...g, ...old, selected: null, history: g.history };
    wtSolitaireSave();
    renderApp();
}
...
function wtSolitaireMoveToColumn(col) {
    ...
    const target = g.tableau[col][g.tableau[col].length - 1];
    if (!wtSolitaireCanTable(cards[0], target)) return;
    wtSolitaireSnapshot();
    wtSolitaireRemoveSelected();
    cards.forEach(c => g.tableau[col].push({ ...c, faceUp: true }));
    g.moves++;
    g.selected = null;
    wtSolitaireSave();
    renderApp();
...
function wtSolitaireMoveToFoundation(suit) {
    ...
    const card = cards[0];
    if (card.suit !== suit || !wtSolitaireCanFoundation(card, g.foundations[suit])) return false;
    wtSolitaireSnapshot();
    wtSolitaireRemoveSelected();
    g.foundations[suit].push({ ...card, faceUp: true });
    g.moves++;
    g.selected = null;
    wtSolitaireSave();
    renderApp();
...
function wtSolitaireHint() {
    ...
    const g = window.wtSolitaire;
    const waste = g.waste[g.waste.length - 1];
    let msg = "Ziehe eine neue Karte vom Stapel oder suche nach einem Ass für die oberen Stapel.";
    if (waste && wtSolitaireCanFoundation(waste, g.foundations[waste.suit])) msg = `Lege ${waste.rank}${waste.suit} oben auf den ${waste.suit}-Stapel.`;
...
document.addEventListener("click", function(event) {
    ...
    if (waste) { g.selected = { type: "waste" }; wtSolitaireSave(); renderApp(); return; }
    const foundation = event.target.closest("[data-wt-foundation]");
    if (foundation && g.selected) { wtSolitaireMoveToFoundation(foundation.dataset.wtFoundation); return; }
    const card = event.target.closest("[data-wt-sol-card]");
    if (card) {
        const col = Number(card.dataset.wtSolCard);
        const index = Number(card.dataset.wtSolIndex);
        const item = g.tableau[col][index];
        if (!item.faceUp && index === g.tableau[col].length - 1) { item.faceUp = true; g.moves++; wtSolitaireSave(); renderApp(); return; }
        if (g.selected) { wtSolitaireMoveToColumn(col); return; }
        g.selected = { type: "tableau", col, index };
        wtSolitaireSave(); renderApp(); return;
    }
    const column = event.target.closest("[data-wt-sol-column]");
    if (column && g.selected) wtSolitaireMoveToColumn(Number(column.dataset.wtSolColumn));
});
...
/* =========================================================
   Solitaire Karten-Auswahl/Verschieben Fix
   ========================================================= */
(function wtSolitaireClickMoveFix() {
    if (window.__wtSolitaireClickMoveFix) return;
    window.__wtSolitaireClickMoveFix = true;

    let touchStartX = 0;
    let touchStartY = 0;
    let touchMoved = false;

    function isSolitaireTarget(target) {
        return !!(target && target.closest && target.closest(".wt-solitaire-game, .wt-home-solitaire-mount, .wt-games-stage"));
    }

    function handleSolitaireAction(event) {
        const target = event.target;
        if (!isSolitaireTarget(target)) return false;

        if (
            target.closest("[onclick*='wtSolitaireHint']") ||
            target.closest("[onclick*='wtSolitaireUndo']") ||
            target.closest("[onclick*='wtSolitaireNewGame']") ||
            target.closest("[onclick*='wtSolitaireDraw']")
        ) {
            return false;
        }

        if (typeof wtSolitaireLoad === "function") wtSolitaireLoad();
        const g = window.wtSolitaire;
        if (!g) return false;

        const foundation = target.closest("[data-wt-foundation]");
        if (foundation && g.selected && typeof wtSolitaireMoveToFoundation === "function") {
            event.preventDefault();
            event.stopPropagation();
            wtSolitaireMoveToFoundation(foundation.dataset.wtFoundation);
            return true;
        }

        const waste = target.closest("[data-wt-waste]");
        if (waste) {
            event.preventDefault();
            event.stopPropagation();
            g.selected = { type: "waste" };
            if (typeof wtSolitaireSave === "function") wtSolitaireSave();
            if (typeof renderApp === "function") renderApp();
            return true;
        }

        const card = target.closest("[data-wt-sol-card]");
        if (card) {
            event.preventDefault();
            event.stopPropagation();
            ...
            const col = Number(card.dataset.wtSolCard);
            const index = Number(card.dataset.wtSolIndex);
            const item = g.tableau && g.tableau[col] ? g.tableau[col][index] : null;
            if (!item) return true;

            if (!item.faceUp && index === g.tableau[col].length - 1) {
                item.faceUp = true;
                g.moves = (g.moves || 0) + 1;
                if (typeof wtSolitaireSave === "function") wtSolitaireSave();
                if (typeof renderApp === "function") renderApp();
            ...
            if (g.selected && typeof wtSolitaireMoveToColumn === "function") {
                wtSolitaireMoveToColumn(col);
                return true;
            }
        ...
        const column = target.closest("[data-wt-sol-column]");
        if (column && g.selected && typeof wtSolitaireMoveToColumn === "function") {
            event.preventDefault();
            event.stopPropagation();
            wtSolitaireMoveToColumn(Number(column.dataset.wtSolColumn));
            return true;
        }
    ...
    document.addEventListener("click", function(event) {
        handleSolitaireAction(event);
    }, true);

    document.addEventListener("touchstart", function(event) {
        if (!isSolitaireTarget(event.target)) return;
        const touch = event.touches && event.touches[0];
        if (!touch) return;
...
/* =========================================================
   Solitaire Auswahl-Fix: andere Karte jederzeit anwählbar
   ========================================================= */
(function wtSolitaireReselectFix() {
    if (window.__wtSolitaireReselectFix) return;
    window.__wtSolitaireReselectFix = true;
    ...
    function selectAndRender(selection) {
        if (typeof wtSolitaireLoad === "function") wtSolitaireLoad();
        if (!window.wtSolitaire) return;
        window.wtSolitaire.selected = selection;
        if (typeof wtSolitaireSave === "function") wtSolitaireSave();
        if (typeof renderApp === "function") renderApp();
    }
    ...
    function clearSelectionAndRender() {
        if (typeof wtSolitaireLoad === "function") wtSolitaireLoad();
        if (!window.wtSolitaire) return;
        window.wtSolitaire.selected = null;
        if (typeof wtSolitaireSave === "function") wtSolitaireSave();
        if (typeof renderApp === "function") renderApp();
    }

    function tryMoveToColumnWithoutSticking(col) {
        const before = JSON.stringify(window.wtSolitaire && window.wtSolitaire.selected);
        if (typeof wtSolitaireMoveToColumn === "function") {
            wtSolitaireMoveToColumn(col);
        }
        const after = JSON.stringify(window.wtSolitaire && window.wtSolitaire.selected);
        // If still the same selection after attempted move, the move failed.
        return before !== after || !(window.wtSolitaire && window.wtSolitaire.selected);
    }

    function tryMoveToFoundationWithoutSticking(suit) {
        const before = JSON.stringify(window.wtSolitaire && window.wtSolitaire.selected);
        let moved = false;
        if (typeof wtSolitaireMoveToFoundation === "function") {
            moved = wtSolitaireMoveToFoundation(suit) === true;
        }
        const after = JSON.stringify(window.wtSolitaire && window.wtSolitaire.selected);
        return moved || before !== after || !(window.wtSolitaire && window.wtSolitaire.selected);
    }

    function handle(event) {
        const target = event.target;
        if (!isSolitaireArea(target)) return;

        // Let action buttons work normally.
        if (
            target.closest("[onclick*='wtSolitaireHint']") ||
            target.closest("[onclick*='wtSolitaireUndo']") ||
            target.closest("[onclick*='wtSolitaireNewGame']") ||
        ...
        if (foundation) {
            ...
            if (g.selected) {
                const moved = tryMoveToFoundationWithoutSticking(foundation.dataset.wtFoundation);
                if (!moved && window.wtSolitaire && window.wtSolitaire.selected) {
        ...
        if (waste) {
            ...
            if (sameSelection(g.selected, next)) {
                clearSelectionAndRender();
            } else {
        ...
        if (card) {
            ...
            // Same selected card: toggle off.
            if (sameSelection(g.selected, clicked)) {
                clearSelectionAndRender();
                return;
            }

            // If another card is selected, first try moving to this card's column.
            // If the move fails, select the newly clicked face-up card instead.
            if (g.selected) {
                const moved = tryMoveToColumnWithoutSticking(col);
                if (!moved && window.wtSolitaire && window.wtSolitaire.selected) {
                    selectAndRender(clicked);
                }
                return;
            }
        ...
        if (column) {
            ...
            if (g.selected) {
                const moved = tryMoveToColumnWithoutSticking(Number(column.dataset.wtSolColumn));
                if (!moved && window.wtSolitaire && window.wtSolitaire.selected) {
                    clearSelectionAndRender();
                }
            }
    ...
    // Capture before the older Solitaire handler so failed moves can be replaced by new selection.
...
/* =========================================================
   Solitaire Direkt-Fix: Karte immer umwählbar
   ========================================================= */
(function wtSolitaireDirectReselectFix() {
    if (window.__wtSolitaireDirectReselectFix) return;
    window.__wtSolitaireDirectReselectFix = true;

    function inSolitaire(target) {
        return !!(target && target.closest && target.closest(".wt-solitaire-game, .wt-home-solitaire-mount, .wt-games-stage"));
    }
    ...
    function moveSelectedToColumn(g, col) {
        ...
        if (!canTable(cards[0], targetTop)) return false;
    ...
    function moveSelectedToFoundation(g, suit) {
        const cards = selectedCards(g);
        if (cards.length !== 1) return false;
    ...
    // Override global move functions so failed moves do NOT keep selection trapped.
    window.wtSolitaireMoveToColumn = function(col) {
        const g = load();
        if (!g || !g.selected) return false;

        const moved = moveSelectedToColumn(g, Number(col));
        if (!moved) {
            // Kein Zug möglich: Auswahl bleibt nur, wenn der Nutzer auf eine leere Spalte geklickt hat.
            // Bei Kartenklick wird unten umgewählt.
            return false;
        }
    ...
    window.wtSolitaireMoveToFoundation = function(suit) {
        const g = load();
        if (!g || !g.selected) return false;

        const moved = moveSelectedToFoundation(g, suit);
    ...
    function handle(event) {
        if (!inSolitaire(event.target)) return;

        // Buttons normal lassen
        if (
        ...
        const wasteBtn = event.target.closest("[data-wt-waste]");
        const foundationBtn = event.target.closest("[data-wt-foundation]");
        const columnBtn = event.target.closest("[data-wt-sol-column]");

        if (!cardBtn && !wasteBtn && !foundationBtn && !columnBtn) return;
        ...
        if (wasteBtn) {
            const next = { type: "waste" };
            g.selected = same(g.selected, next) ? null : next;
            saveRender();
            return;
        }
        ...
        if (cardBtn) {
            ...
            // Gleiche Karte: abwählen.
            if (same(g.selected, clicked)) {
                g.selected = null;
                saveRender();
                return;
            }

            // Wenn schon eine Karte ausgewählt ist:
            // Erst Zug auf diese Spalte versuchen. Wenn nicht möglich: neue Karte auswählen.
            if (g.selected) {
                const moved = moveSelectedToColumn(g, col);
                if (!moved) {
                    g.selected = clicked;
                }
                saveRender();
            ...
            // Keine Auswahl: diese Karte auswählen.
            g.selected = clicked;
            saveRender();
            return;
        }
        ...
        if (foundationBtn) {
            ...
            const moved = moveSelectedToFoundation(g, foundationBtn.dataset.wtFoundation);
            if (!moved) g.selected = null;
            saveRender();
            return;
        }
        ...
        if (columnBtn) {
            if (!g.selected) return;
            const moved = moveSelectedToColumn(g, Number(columnBtn.dataset.wtSolColumn));
            if (!moved) g.selected = null;
            saveRender();
        }
    }

    // Alte Handler überschreiben: dieser läuft zuerst und stoppt danach sofort.
    document.addEventListener("click", handle, true);
