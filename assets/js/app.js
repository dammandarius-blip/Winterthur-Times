...
let deletedCommunityImageIds = [];
window.deletedCommunityImageIds = deletedCommunityImageIds;
...
function wtShouldDeferRemoteRender() {
    ...
    return isMenuOpen || authModalOpen || wtIsEditorProtectingLocalWork();
}
...
/* =========================================================
   User-Persistenz + Ressort-Sync Helper
   ========================================================= */
function wtUserKeyForMerge(user) {
    if (!user || typeof user !== "object") return "";
    return String(user.uid || user.id || user.email || user.username || "").trim().toLowerCase();
}

function wtMergeRegisteredUsersNoDelete(remoteUsers, localUsers, userRoles) {
    const map = new Map();

    function add(user, source) {
        if (!user || typeof user !== "object") return;
        const key = wtUserKeyForMerge(user);
        if (!key) return;
        ...
        if (userRoles && userRoles[user.username]) merged.role = userRoles[user.username];
        if (userRoles && userRoles[user.email]) merged.role = userRoles[user.email];
        if (!merged.role) merged.role = "user";
        if (!merged.subscriptions) merged.subscriptions = { categories: [], authors: [] };
        if (!Array.isArray(merged.subscriptions.categories)) merged.subscriptions.categories = [];
        if (!Array.isArray(merged.subscriptions.authors)) merged.subscriptions.authors = [];

        map.set(key, merged);
    }

    (Array.isArray(localUsers) ? localUsers : []).forEach(u => add(u, "local"));
    (Array.isArray(remoteUsers) ? remoteUsers : []).forEach(u => add(u, "remote"));

    return Array.from(map.values());
}

function wtBackupRegisteredUsers() {
    try {
        if (Array.isArray(registeredUsers) && registeredUsers.length) {
            localStorage.setItem("wt_registered_users_never_delete_backup", JSON.stringify(registeredUsers));
        }
    } catch (_) {}
}

function wtRestoreRegisteredUsersBackupIfNeeded() {
    try {
        if (Array.isArray(registeredUsers) && registeredUsers.length) return false;
        const backup = JSON.parse(localStorage.getItem("wt_registered_users_never_delete_backup") || "[]");
        if (Array.isArray(backup) && backup.length) {
            registeredUsers = backup;
...
function wtApplyCategoriesEverywhere(newCategories) {
    if (!Array.isArray(newCategories)) return;
    const cleaned = Array.from(new Set(newCategories.map(c => String(c || "").trim()).filter(Boolean)));
    ...
    try { window.dispatchEvent(new CustomEvent("wtCategoriesUpdated", { detail: { categories: categories.slice() } })); } catch (_) {}

    document.querySelectorAll("select").forEach(select => {
        const id = String(select.id || "").toLowerCase();
        const name = String(select.name || "").toLowerCase();
        const text = String(select.closest("label, div, p, section")?.textContent || "").toLowerCase();
        if (!id.includes("category") && !id.includes("ressort") && !name.includes("category") && !text.includes("ressort") && !text.includes("kategorie")) return;

        const current = select.value;
        const oldOptions = Array.from(select.options || []).map(o => o.value).filter(Boolean);
        const looksLikeCategorySelect = oldOptions.some(v => categories.includes(v)) || id.includes("category") || text.includes("ressort");
        if (!looksLikeCategorySelect) return;

        select.innerHTML = categories.map(cat => `<option value="${cat.replace(/"/g, "&quot;")}">${cat}</option>`).join("");
        ...
        articlesDoc.onSnapshot({ includeMetadataChanges: true }, (snap) => {
            ...
            try {
                ...
                if (Array.isArray(data.categories)) {
                    wtApplyCategoriesEverywhere(data.categories);
                }
                if (Array.isArray(data.deletedCommunityImageIds) && typeof window.wtMergeDeletedCommunityImageIds === "function") window.wtMergeDeletedCommunityImageIds(data.deletedCommunityImageIds);
                if (Array.isArray(data.communityImages)) {
                    const deleted = (() => { try { return JSON.parse(localStorage.getItem("wt_gallery_deleted_ids_v2") || localStorage.getItem("wt_gallery_deleted_ids_v1") || "[]").map(String); } catch (_) { return []; } })();
                    communityImages = data.communityImages.filter(img => img && !img.isDeleted && !deleted.includes(String(img.id)));
        ...
        usersDoc.onSnapshot({ includeMetadataChanges: true }, (snap) => {
            ...
            try {
                if (Array.isArray(data.registeredUsers)) {
                    registeredUsers = wtMergeRegisteredUsersNoDelete(data.registeredUsers, registeredUsers, data.userRoles || null);
                    window.registeredUsers = registeredUsers;
                    ensureAllUsersSubscriptions();
                    applySavedUserRolesToRegisteredUsers(data.userRoles || null);
                    wtBackupRegisteredUsers();
                } else {
                    wtRestoreRegisteredUsersBackupIfNeeded();
                }
            } finally {
                isApplyingRemoteState = false;
            }
...
function wtArticleMatchesSearch(article, query) {
    const terms = wtExpandedSearchTerms(query);
    ...
    return terms.some(term => {
        ...
        return words.some(word => {
            ...
            if (term.length >= 5 && word.length >= 5 && word.slice(0, 5) === term.slice(0, 5)) return true;
            return false;
        });
    });
}
...
function renderSearchResults() {
    ...
    if (searchCategory) {
        ...
        results = currentArticles.filter(a => wtArticleMatchesSearch(a, query));
...
/* Benutzer-Speicher Wrapper Fallback */
(function wtWrapSaveUsersNoDeleteFallback() {
    if (window.__wtWrapSaveUsersNoDeleteFallback) return;
    window.__wtWrapSaveUsersNoDeleteFallback = true;
    try {
        const old = window.saveUsersNow;
        if (typeof old === "function") {
            window.saveUsersNow = async function(...args) {
                wtRestoreRegisteredUsersBackupIfNeeded();
...
/* Laufender Schutz: Benutzer nie durch leere Snapshots verlieren + Ressorts aktuell halten */
(function wtUserAndCategoryRuntimeGuard() {
    if (window.__wtUserAndCategoryRuntimeGuard) return;
    window.__wtUserAndCategoryRuntimeGuard = true;

    setInterval(() => {
        try {
            if (Array.isArray(registeredUsers) && registeredUsers.length) wtBackupRegisteredUsers();
            else if (wtRestoreRegisteredUsersBackupIfNeeded() && typeof renderApp === "function") renderApp();
        } catch (_) {}

        try {
            if (Array.isArray(categories) && categories.length) wtApplyCategoriesEverywhere(categories);
            else {
                const backup = JSON.parse(localStorage.getItem("wt_categories_backup") || "[]");
                if (Array.isArray(backup) && backup.length) wtApplyCategoriesEverywhere(backup);
            }
        } catch (_) {}
    }, 2000);
})();
...
/* =========================================================
   Tagesbilder wirklich dauerhaft löschen Fix
   ========================================================= */
(function wtGalleryPermanentDeleteFix() {
    if (window.__wtGalleryPermanentDeleteFix) return;
    window.__wtGalleryPermanentDeleteFix = true;

    const LOCAL_IMAGES_KEY = "wt_gallery_local_fallback_images_v1";
    const LOCAL_DELETED_KEY = "wt_gallery_deleted_ids_v1";
    const LOCAL_LIKES_KEYS = [
        "wt_gallery_local_likes_v1",
        "wt_gallery_like_state_final_v1",
        "wt_gallery_likes_final2"
    ];
    ...
    function writeJson(key, value) {
        try {
            ...
            console.warn("Lokale Galerie-Daten konnten nicht gespeichert werden:", err);
        }
    }

    function deletedIds() {
        const arr = readJson(LOCAL_DELETED_KEY, []);
        return Array.isArray(arr) ? arr.map(String) : [];
    }

    function rememberDeletedId(id) {
        const idStr = String(id);
        const list = deletedIds().filter(x => x !== idStr);
        list.unshift(idStr);
        writeJson(LOCAL_DELETED_KEY, list.slice(0, 500));
    }
    ...
    function removeFromMemory(id) {
        ...
        arrays().forEach(arr => {
            for (let i = arr.length - 1; i >= 0; i--) {
                if (String(arr[i] && arr[i].id) === idStr) {
                    arr.splice(i, 1);
                }
            }
        });
    ...
    function removeFromLocalFallback(id) {
        const idStr = String(id);
        const local = readJson(LOCAL_IMAGES_KEY, []);
        if (Array.isArray(local)) {
            writeJson(LOCAL_IMAGES_KEY, local.filter(img => String(img && img.id) !== idStr));
        }
    }

    function removeLikes(id) {
        const idStr = String(id);
        LOCAL_LIKES_KEYS.forEach(key => {
            const map = readJson(key, {});
            if (map && typeof map === "object" && idStr in map) {
                delete map[idStr];
                writeJson(key, map);
            }
        });
    ...
    async function saveCommunityImagesAfterDelete() {
        const images = arrays()[0] || [];

        // Wirklich ohne gelöschte IDs speichern.
        const clean = images.filter(img => img && !isDeletedId(img.id) && !img.isDeleted);

        try {
            if (typeof communityImages !== "undefined" && Array.isArray(communityImages)) {
                communityImages.splice(0, communityImages.length, ...clean);
            }
        } catch (_) {}

        if (Array.isArray(window.communityImages)) {
            window.communityImages.splice(0, window.communityImages.length, ...clean);
        }

        // Direkt in Firebase schreiben, falls verfügbar.
        try {
            if (typeof firebaseDb !== "undefined" && firebaseDb && typeof firebase !== "undefined") {
                await firebaseDb.collection("data").doc("articles").set({
                    communityImages: clean,
                    deletedCommunityImageIds: deletedIds(),
                    updatedAt: firebase.firestore.FieldValue.serverTimestamp()
                }, { merge: true });
                return true;
            }
        } catch (err) {
            console.warn("Direktes Firebase-Löschen fehlgeschlagen:", err);
        }

        // Normale Save-Funktion als Fallback
        try {
            if (typeof window.saveState === "function") {
                await window.saveState();
                return true;
            ...
            console.warn("saveState nach Bildlöschung fehlgeschlagen:", err);
        }

        return false;
    }

    async function permanentlyDeleteImage(id) {
        const idStr = String(id);

        rememberDeletedId(idStr);
        removeFromMemory(idStr);
        removeFromLocalFallback(idStr);
        removeLikes(idStr);

        await saveCommunityImagesAfterDelete();

        if (typeof renderApp === "function") renderApp();
    }

    // Globale Löschfunktion ersetzen.
    window.deleteCommunityImage = function(id) {
        const idStr = String(id);

        currentModal = {
            title: "Bild endgültig löschen?",
            message: "Dieses Tagesbild wird dauerhaft aus der Galerie entfernt. Es sollte danach nicht mehr zurückkommen.",
            onConfirm: async function() {
                currentModal = null;
                await permanentlyDeleteImage(idStr);
            }
        };
    ...
    // Beim Rendern/Refresh: gelöschte IDs immer rausfiltern, damit alte Firebase-/Local-Snapshots sie nicht zurückbringen.
    function filterDeletedEverywhere() {
        arrays().forEach(arr => {
            for (let i = arr.length - 1; i >= 0; i--) {
                const img = arr[i];
                if (!img || img.isDeleted || isDeletedId(img.id)) {
                    arr.splice(i, 1);
                }
            }
        });
        ...
        const local = readJson(LOCAL_IMAGES_KEY, []);
        if (Array.isArray(local)) {
            const cleanLocal = local.filter(img => img && !img.isDeleted && !isDeletedId(img.id));
            if (cleanLocal.length !== local.length) writeJson(LOCAL_IMAGES_KEY, cleanLocal);
        }
    }

    // Lokale Fallback-Merge-Funktionen dürfen gelöschte Bilder nicht wieder einfügen.
    const oldRender = window.renderApp;
    if (typeof oldRender === "function" && !oldRender.__wtPermanentDeletePatched) {
        const patched = function(...args) {
            filterDeletedEverywhere();
            return oldRender.apply(this, args);
        };
        patched.__wtPermanentDeletePatched = true;
        window.renderApp = patched;
    }

    // Falls aus Firebase wieder ein alter Stand kommt, kurz danach wieder entfernen und speichern.
    let lastDeletedSignature = "";
    setInterval(() => {
        const sig = deletedIds().join("|");
        filterDeletedEverywhere();

        if (sig && sig !== lastDeletedSignature) {
            lastDeletedSignature = sig;
            saveCommunityImagesAfterDelete().catch(() => {});
        }
    }, 1000);

    // Click-Fallback: falls alte Inline-onclicks String/Number-Probleme machen.
    document.addEventListener("click", event => {
        const btn = event.target.closest && event.target.closest("button[onclick*='deleteCommunityImage']");
        if (!btn) return;

        const onclick = btn.getAttribute("onclick") || "";
        const match = onclick.match(/deleteCommunityImage\(['"]?([^'")]+)['"]?\)/);
        if (!match) return;

        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();
        window.deleteCommunityImage(match[1]);
    }, true);
...
/* Gelöschte Tagesbild-IDs aus Firebase übernehmen */
(function wtDeletedGalleryIdsRemoteSync() {
    if (window.__wtDeletedGalleryIdsRemoteSync) return;
    window.__wtDeletedGalleryIdsRemoteSync = true;

    const KEY = "wt_gallery_deleted_ids_v1";

    window.wtMergeDeletedCommunityImageIds = function(ids) {
        if (!Array.isArray(ids)) return;
        try {
            const local = JSON.parse(localStorage.getItem(KEY) || "[]");
            const merged = Array.from(new Set([...(Array.isArray(local) ? local : []), ...ids].map(String)));
...
/* =========================================================
   FINAL: Tagesbilder wirklich komplett löschen
   ========================================================= */
(function wtFinalGalleryHardDelete() {
    if (window.__wtFinalGalleryHardDelete) return;
    window.__wtFinalGalleryHardDelete = true;

    const DELETED_KEY = "wt_gallery_deleted_ids_v2";
    const LOCAL_KEYS = [
        "wt_gallery_local_fallback_images_v1",
        "wt_gallery_local_likes_v1",
        "wt_gallery_like_state_final_v1",
        "wt_gallery_likes_final2"
    ];
    ...
    function getDeletedIds() {
        const a = readJson(DELETED_KEY, []);
        const b = readJson("wt_gallery_deleted_ids_v1", []);
        const c = (() => { try { return Array.isArray(deletedCommunityImageIds) ? deletedCommunityImageIds : []; } catch (_) { return []; } })();
        return Array.from(new Set([...(Array.isArray(a)?a:[]), ...(Array.isArray(b)?b:[]), ...(Array.isArray(c)?c:[])].map(String)));
    }

    function setDeletedIds(ids) {
        const clean = Array.from(new Set((ids || []).map(String))).slice(0, 1000);
        writeJson(DELETED_KEY, clean);
        writeJson("wt_gallery_deleted_ids_v1", clean);
        try {
            deletedCommunityImageIds = clean;
            window.deletedCommunityImageIds = clean;
        } catch (_) {
            window.deletedCommunityImageIds = clean;
        }
        return clean;
    ...
    function rememberDeleted(id) {
        ...
        return setDeletedIds(ids);
    }

    function imageArrays() {
        const arrays = [];
        try { if (Array.isArray(communityImages)) arrays.push(communityImages); } catch (_) {}
        if (Array.isArray(window.communityImages) && !arrays.includes(window.communityImages)) arrays.push(window.communityImages);
        return arrays;
    }
    ...
    function removeEverywhere(id) {
        ...
        imageArrays().forEach(arr => {
            for (let i = arr.length - 1; i >= 0; i--) {
                if (String(arr[i] && arr[i].id) === idStr || arr[i]?.isDeleted) arr.splice(i, 1);
            }
        });

        // lokale Fallbackbilder entfernen
        const local = readJson("wt_gallery_local_fallback_images_v1", []);
        if (Array.isArray(local)) {
            writeJson("wt_gallery_local_fallback_images_v1", local.filter(img => String(img && img.id) !== idStr && !img?.isDeleted));
        }

        // Like-Maps bereinigen
        ["wt_gallery_local_likes_v1","wt_gallery_like_state_final_v1","wt_gallery_likes_final2"].forEach(key => {
            const map = readJson(key, {});
    ...
    function filterDeletedEverywhere() {
        ...
        imageArrays().forEach(arr => {
            for (let i = arr.length - 1; i >= 0; i--) {
                const img = arr[i];
                if (!img || img.isDeleted || deleted.has(String(img.id))) arr.splice(i, 1);
            }
        });

        const local = readJson("wt_gallery_local_fallback_images_v1", []);
        if (Array.isArray(local)) {
            const clean = local.filter(img => img && !img.isDeleted && !deleted.has(String(img.id)));
            if (clean.length !== local.length) writeJson("wt_gallery_local_fallback_images_v1", clean);
        }
    }

    async function saveHardDeleteToFirebase() {
        filterDeletedEverywhere();
        const clean = (imageArrays()[0] || []).filter(img => img && !img.isDeleted && !getDeletedIds().includes(String(img.id)));
        ...
        try {
            if (typeof firebaseDb !== "undefined" && firebaseDb && typeof firebase !== "undefined") {
                await firebaseDb.collection("data").doc("articles").set({
                    communityImages: clean,
                    deletedCommunityImageIds: deleted,
                    updatedAt: firebase.firestore.FieldValue.serverTimestamp()
                }, { merge: true });
                return true;
            }
        } catch (err) {
            console.warn("Hard delete Firebase speichern fehlgeschlagen:", err);
        }
    ...
    window.wtMergeDeletedCommunityImageIds = function(ids) {
        if (!Array.isArray(ids)) return;
        setDeletedIds([...getDeletedIds(), ...ids]);
        filterDeletedEverywhere();
    };

    window.wtHardDeleteCommunityImage = async function(id) {
        const idStr = String(id);
        rememberDeleted(idStr);
        removeEverywhere(idStr);
        filterDeletedEverywhere();
        await saveHardDeleteToFirebase();
        filterDeletedEverywhere();
        if (typeof renderApp === "function") renderApp();
    };

    // Die sichtbare Löschfunktion wird direkt ersetzt.
    window.deleteCommunityImage = function(id) {
        const idStr = String(id);
        currentModal = {
            title: "Tagesbild endgültig löschen?",
            message: "Dieses Bild wird dauerhaft entfernt und kommt auch nach dem Aktualisieren nicht zurück.",
            onConfirm: async function() {
                currentModal = null;
                await window.wtHardDeleteCommunityImage(idStr);
            }
        };
        if (typeof renderApp === "function") renderApp();
    };

    // Alte Inline-Klicks abfangen
    document.addEventListener("click", function(event) {
        const btn = event.target.closest && event.target.closest("button[onclick*='deleteCommunityImage']");
        if (!btn) return;
        const raw = btn.getAttribute("onclick") || "";
        const m = raw.match(/deleteCommunityImage\(['"]?([^'")]+)['"]?\)/);
        if (!m) return;
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();
        window.deleteCommunityImage(m[1]);
    }, true);

    // Nach jedem Render alte Snapshots rauswerfen.
    const oldRender = window.renderApp;
    if (typeof oldRender === "function" && !oldRender.__wtHardDeleteWrapped) {
        const wrapped = function(...args) {
            filterDeletedEverywhere();
            const r = oldRender.apply(this, args);
            setTimeout(filterDeletedEverywhere, 0);
            return r;
        };
        wrapped.__wtHardDeleteWrapped = true;
...
/* FINAL: Ressorts überall automatisch aktualisieren */
(function wtFinalCategoriesLiveEverywhere() {
    if (window.__wtFinalCategoriesLiveEverywhere) return;
    window.__wtFinalCategoriesLiveEverywhere = true;

    function cats() {
        try { if (Array.isArray(categories)) return categories; } catch (_) {}
        if (Array.isArray(window.categories)) return window.categories;
        try {
            const b = JSON.parse(localStorage.getItem("wt_categories_backup") || "[]");
            if (Array.isArray(b)) return b;
        } catch (_) {}
        return [];
    }

    window.wtApplyCategoriesEverywhere = function(newCats) {
        const clean = Array.from(new Set((newCats || cats()).map(c => String(c || "").trim()).filter(Boolean)));
        if (!clean.length) return;
        try { categories = clean; } catch (_) {}
        window.categories = clean;
        try { localStorage.setItem("wt_categories_backup", JSON.stringify(clean)); } catch (_) {}

        document.querySelectorAll("select").forEach(sel => {
            const id = String(sel.id || "").toLowerCase();
            const text = String(sel.closest("label,div,p,section")?.textContent || "").toLowerCase();
            const isCat = id.includes("category") || id.includes("ressort") || text.includes("ressort") || text.includes("kategorie");
            if (!isCat) return;
            const current = sel.value;
            sel.innerHTML = clean.map(c => `<option value="${c.replace(/"/g, "&quot;")}">${c}</option>`).join("");
            if (clean.includes(current)) sel.value = current;
        });
    };

    window.addEventListener("wtCategoriesUpdated", e => window.wtApplyCategoriesEverywhere(e.detail?.categories || cats()));
    setInterval(() => window.wtApplyCategoriesEverywhere(cats()), 1200);
})();
