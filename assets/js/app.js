...
const crosswordSets = [
    {
        title: "Winterthur Times",
        rows: 9,
        cols: 9,
        words: [
            { id: 1, number: 1, clue: "Text in einer Zeitung", answer: "ARTIKEL", row: 1, col: 1, dir: "across" },
            { id: 2, number: 2, clue: "Person, die Artikel schreibt", answer: "AUTOR", row: 1, col: 1, dir: "down" },
            { id: 3, number: 3, clue: "Lebewesen, nicht Pflanze", answer: "TIER", row: 3, col: 1, dir: "across" },
            { id: 4, number: 4, clue: "Anklickbarer Verweis", answer: "LINK", row: 1, col: 7, dir: "down" },
            { id: 5, number: 5, clue: "Bereich wie Politik, Sport oder Kultur", answer: "RESSORT", row: 5, col: 1, dir: "across" },
            { id: 6, number: 6, clue: "Geschriebener Inhalt", answer: "TEXT", row: 5, col: 7, dir: "down" }
        ]
    },
    {
        title: "Schule & Wissen",
        ...
        cols: 9,
        words: [
            { id: 1, number: 1, clue: "Fach mit Zahlen", answer: "MATHE", row: 0, col: 0, dir: "across" },
            { id: 2, number: 2, clue: "Darin sammelt man Blätter", answer: "MAPPE", row: 0, col: 0, dir: "down" },
            { id: 3, number: 3, clue: "Kurze Erholung zwischen Lektionen", answer: "PAUSE", row: 2, col: 0, dir: "across" },
            { id: 4, number: 4, clue: "Tier mit langen Ohren", answer: "HASE", row: 0, col: 3, dir: "down" },
            { id: 5, number: 5, clue: "Kleines Mäppchen", answer: "ETUI", row: 3, col: 3, dir: "across" },
            { id: 6, number: 6, clue: "Einfälle oder Gedanken", answer: "IDEEN", row: 3, col: 6, dir: "down" }
        ]
    },
    {
        title: "Alltag & Logik",
        ...
        cols: 9,
        words: [
            { id: 1, number: 1, clue: "Zeigt die Uhrzeit", answer: "UHR", row: 0, col: 0, dir: "across" },
            { id: 2, number: 2, clue: "Fällt aus Wolken", answer: "REGEN", row: 0, col: 2, dir: "down" },
            { id: 3, number: 3, clue: "Fährt auf Schienen", answer: "ZUG", row: 2, col: 0, dir: "across" },
            { id: 4, number: 4, clue: "Wolke am Boden", answer: "NEBEL", row: 4, col: 2, dir: "across" },
            { id: 5, number: 5, clue: "Leuchtet am Tag", answer: "SONNE", row: 6, col: 0, dir: "across" },
            { id: 6, number: 6, clue: "Man liest darin Nachrichten", answer: "ZEITUNG", row: 0, col: 7, dir: "down" }
        ]
    }
];
...
function buildCrosswordCells(set) {
    ...
    set.words.forEach(word => {
        ...
        letters.forEach((letter, offset) => {
            ...
            if (offset === 0) numbers[key] = word.number;
        });
    });
...
function renderCrossword(prefix = "crosswordHome") {
    ...
    const { cells, numbers } = buildCrosswordCells(set);
    const isFullscreen = String(prefix).toLowerCase().includes("full");
                        ...
                        ${Array.from({ length: set.rows * set.cols }, (_, i) => {
                            ...
                            return `
                                <div class="wt-crossword-square">
                                    ${numbers[key] ? `<span class="wt-crossword-number">${numbers[key]}</span>` : ""}
                                    <input id="${prefix}_${key}" maxlength="1" value="${escapePuzzleValue(value)}" class="wt-crossword-cell" oninput="saveCrosswordCell('${prefix}', '${key}')" />
                                </div>`;
                        }).join("")}
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div>
                            <h4 class="font-black text-blue-200 uppercase tracking-wide mb-2">Waagrecht</h4>
                            <ol class="space-y-2">
                                ${across.map(w => `<li><span class="font-black text-white">${w.number}.</span> ${w.clue}</li>`).join("")}
                            </ol>
                        </div>
                        <div>
                            <h4 class="font-black text-blue-200 uppercase tracking-wide mb-2">Senkrecht</h4>
                            <ol class="space-y-2">
                                ${down.map(w => `<li><span class="font-black text-white">${w.number}.</span> ${w.clue}</li>`).join("")}
                            </ol>
                        </div>
                    </div>
                </div>
