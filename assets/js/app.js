...
function renderFooter() {
    const footerCategories = Array.from(new Set((Array.isArray(categories) ? categories : [])
        .map(cat => String(cat || "").trim())
        .filter(Boolean)));
    const escapeFooterText = value => String(value || "")
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
        .replace(/"/g, "&quot;")
            ...
            <div>
                ...
                <ul class="flex flex-col gap-2 text-sm text-gray-400">
                    ${footerCategories.map(cat => {
                        const safeCat = escapeFooterText(cat);
...
window.wtGetPublicSupportContext = function() {
    const activeCategories = Array.from(new Set((Array.isArray(categories) ? categories : [])
        .map(cat => String(cat || "").trim())
        .filter(Boolean)));
