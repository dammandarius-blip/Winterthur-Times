...
function mergeRegisteredUsersWithoutAutoDelete(localUsers, remoteUsers) {
    ...
    const deletedKeys = new Set(getDeletedUserKeys());

    const isDeletedByAdmin = user => getStableUserKeys(user).some(key => deletedKeys.has(key));

    if (remote.length === 0 && local.length > 0) {
        return local.filter(user => !isDeletedByAdmin(user)).map(user => ensureUserSubscriptions({ ...user }));
    }

    const byKey = new Map();
    local.forEach(user => {
        if (isDeletedByAdmin(user)) return;
...
async function saveUsersNow() {
    ...
    if (isFirebaseConnected && firebaseDb && typeof firebase !== "undefined") {
        ...
        await firebaseDb.collection("data").doc("users").set({
            ...
            deletedUserKeys: getDeletedUserKeys(),
            updatedAt: firebase.firestore.FieldValue.serverTimestamp()
        }, { merge: true });
    } else {
        window.saveState();
        ...
        usersDoc.onSnapshot({ includeMetadataChanges: true }, (snap) => {
            ...
            try {
                if (Array.isArray(data.registeredUsers)) {
                    if (Array.isArray(data.deletedUserKeys)) {
                        writeDeletedUserKeys([...getDeletedUserKeys(), ...data.deletedUserKeys]);
                    }
