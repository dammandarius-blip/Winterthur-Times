...
function getSmartSearchWords(query) {
    const base = normalizeSmartSearchText(query).split(" ").filter(Boolean);
    const map = {
        wohnen: ["wohnung", "wohnungen", "wohnraum", "miete", "mieten", "mietzins", "bezahlbar", "guenstig", "günstig", "preisguenstig", "preisgünstig"],
        ...
        flugzeug: ["flugzeuge", "flug", "fliegen", "airbus", "boeing", "airport", "flughafen"],
        flug: ["flugzeug", "flugzeuge", "airbus", "boeing", "airport", "flughafen"],
        spiel: ["spiele", "sudoku", "kreuzwortraetsel", "kreuzworträtsel", "solitaire", "raetsel", "rätsel"]
    };
    const out = new Set(base);
    base.forEach(w => {
        (map[w] || []).forEach(x => out.add(normalizeSmartSearchText(x)));
        if (w.endsWith("en") && w.length > 5) out.add(w.slice(0, -2));
        if (w.endsWith("ungen") && w.length > 8) out.add(w.slice(0, -5));
        if (w.endsWith("ung") && w.length > 6) out.add(w.slice(0, -3));
    });
    return Array.from(out).filter(Boolean);
}

function smartArticleMatches(article, query) {
    const words = getSmartSearchWords(query);
    if (!words.length) return true;
    const text = normalizeSmartSearchText([
    ...
    return words.some(q =>
        text.includes(q) ||
        textWords.some(w => w.startsWith(q) || q.startsWith(w) || (q.length >= 5 && Math.abs(q.length - w.length) <= 2 && (w.includes(q.slice(0, 5)) || q.includes(w.slice(0, 5)))))
    );
}
...
/* Handy-Menü Fix */
(function wtMobileMenuFix() {
    if (window.__wtMobileMenuFix) return;
    window.__wtMobileMenuFix = true;
    ...
    function openFallbackMenu() {
        ...
        overlay.innerHTML = `
            <div class="wt-mobile-menu-panel">
                <div class="wt-mobile-menu-head"><strong>MENÜ</strong><button type="button" data-wt-menu-close>×</button></div>
                <button type="button" data-wt-home>Startseite</button>
                <button type="button" data-wt-gallery>Tagesbilder</button>
                ${cats.map(c => `<button type="button" data-wt-cat="${c}">${c}</button>`).join("")}
            </div>`;
    ...
    document.addEventListener("click", function(event) {
        ...
        if (menuButton && menuButton.getAttribute("onclick") && menuButton.getAttribute("onclick").includes("toggleMenu")) {
            setTimeout(() => {
                if (!document.querySelector(".fixed.inset-0.bg-black\\/60")) {
                    openFallbackMenu();
                }
            }, 120);
        }
        ...
        if (cat) {
            document.getElementById("wtMobileMenuFallback")?.remove();
            executeSearchCategory(cat.dataset.wtCat);
        }
    }, true);
})();
