...
window.forgotPassword = function() {
    try {
        ...
        // Versuche die E-Mail anhand des Benutzernamens zu ermitteln oder nutze sie direkt
        let email = '';
        ...
        // Online-Fallback: Firebase nutzen, wenn vorhanden
        if (isFirebaseConnected && firebaseAuth) {
            firebaseAuth.sendPasswordResetEmail(email)
                .then(() => {
                    showModal('Passwort zurücksetzen', `Ein Link zum Zurücksetzen des Passworts wurde an ${email} gesendet. Bitte prüfe dein Postfach.`);
                })
                .catch((err) => {
                    console.error('Passwort-Reset über Firebase fehlgeschlagen:', err);
                    showWarning('Der Passwort-Reset konnte nicht gesendet werden. Bitte versuche es später erneut.');
                });
            return;
        }
        // Lokal: Fallback über Worker-API anfragen
        fetch(`${WT_WORKER_BASE}/api/forgot-password`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            ...
            .then(() => {
                showModal('Passwort zurücksetzen', `Ein Link zum Zurücksetzen des Passworts wurde an ${email} gesendet. Bitte prüfe dein Posteingang.`);
            })
            .catch((err) => {
                console.error('Passwort-Reset via Worker fehlgeschlagen:', err);
                showWarning('Der Passwort-Reset konnte nicht gesendet werden. Bitte versuche es später erneut.');
            });
    } catch (err) {
