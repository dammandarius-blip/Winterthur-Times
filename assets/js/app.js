...
function renderArticlePoll(article) {
    ...
    return `
        <section class="article-poll mt-10 mb-8 p-5 rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <p class="mb-4 text-slate-700">${escapeArticleHtml(poll.question)}</p>
...
document.addEventListener('click', async (event) => {
    ...
    const voterId = getUserId ? getUserId() : (localStorage.getItem('wt_user_id') || 'guest');
...
/* =========================================================
   ...
   Erstellt/aktualisiert Artikel mit Inline-Bildern und Umfragen.
   ========================================================= */
...
(function initAdvancedArticleEditor() {
    ...
    function isAdminVisible() {
        ...
        return txt.includes("admin") || txt.includes("artikel erstellen") || txt.includes("artikel verwalten") || location.hash.toLowerCase().includes("admin");
    }
    ...
    function mountEditorWhenPossible() {
        ...
        wrapper.className = "wt-advanced-editor";
            ...
            <div class="wt-editor-card">
                <div class="wt-editor-head">
                    <div>
                        ...
                        <h2>Artikel mit Zwischenbildern & Umfrage erstellen</h2>
                        <p>Hier kannst du komplette Artikel direkt korrekt für Firebase speichern.</p>
                    </div>
    ...
    function buildArticleFromForm() {
        ...
        const sourceLines = (document.getElementById("wtArticleSources")?.value || "")
            .split(/\n+/)
            .map(s => s.trim())
            .filter(Boolean);
    ...
    async function saveArticleToFirebase(article) {
        if (typeof articles === "undefined") {
            throw new Error("Die globale Artikelliste wurde nicht gefunden. Bist du im richtigen Adminbereich?");
        }
        ...
        if (typeof saveData === "function") {
            ...
            await db.collection("data").doc("articles").set({ articles }, { merge: true });
        } else {
    ...
    function bindAdvancedEditor(wrapper) {
        ...
        wrapper.querySelector("#wtSaveArticle")?.addEventListener("click", async () => {
            try {
                ...
                preview.hidden = false;
                preview.textContent = JSON.stringify(article, null, 2);
                status.textContent = "Artikel wurde gespeichert. Er sollte jetzt auf der Website sichtbar sein.";
                status.className = "wt-editor-status ok";
            } catch (err) {
