        ...
        <section class="bg-gradient-to-br from-blue-950 via-blue-900 to-slate-900 text-white p-6 border border-blue-800 shadow-sm rounded-sm font-sans relative overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <div class="flex flex-col sm:flex-row gap-2">
                    <input id="dailyRiddleAnswer" type="text" placeholder="Antwort eingeben..." ${solved ? "disabled" : ""}
                        ...
                        onkeydown="if(event.key === 'Enter') checkDailyRiddleAnswer();" />
                    <button onclick="checkDailyRiddleAnswer()" ${solved ? "disabled" : ""}
                ...
                <div class="flex items-center justify-between gap-3 mt-3">
                    <button onclick="showDailyRiddleHint()" class="text-xs text-blue-200 hover:text-white hover:underline">Hinweis anzeigen</button>
                    <span class="text-[11px] text-blue-200">Neues Rätsel jeden Tag</span>
                </div>
                <p id="dailyRiddleFeedback" class="mt-3 text-sm font-bold ${solved ? "text-green-300" : "text-blue-100"}">${solved ? "Richtig gelöst. Morgen gibt es ein neues Rätsel." : ""}</p>
            </div>
        </section>
    `;
}

function checkDailyRiddleAnswer() {
    const input = document.getElementById('dailyRiddleAnswer');
    const feedback = document.getElementById('dailyRiddleFeedback');
...
function renderHome() {
    if (articles.length === 0) return `<div class="max-w-3xl mx-auto flex flex-col gap-6">${renderDailyRiddle()}<div class="text-center py-8 text-gray-500 font-sans">Keine Artikel vorhanden.</div></div>`;
    
    const now = new Date();
    const currentArticles = hasAuthorAccess() ? articles : articles.filter(a => !a.autoDeleteDate || new Date(a.autoDeleteDate) > now);

    if (currentArticles.length === 0) return `<div class="max-w-3xl mx-auto flex flex-col gap-6">${renderDailyRiddle()}<div class="text-center py-8 text-gray-500 font-sans">Derzeit gibt es keine sichtbaren Artikel.</div></div>`;
