...
(function initHomeOnlySolitaire() {
    ...
    function isHomePage() {
        ...
        const isAdmin = path.includes("admin") || document.body?.classList.contains("admin-page");
        if (isAdmin) return false;

        const hash = (location.hash || "").toLowerCase();
        if (hash && !["#", "#/", "#home", "#start", "#startseite"].includes(hash)) return false;

        return path.endsWith("/") || path.endsWith("/index.html") || path.includes("/winterthur-times/");
    }
