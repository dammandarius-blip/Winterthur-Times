...
function wtExpandedSearchTerms(query) {
    ...
    base.forEach(term => {
        (related[term] || []).forEach(x => out.add(wtNormalizeSearch(x)));
        if (term.endsWith("en") && term.length > 5) out.add(term.slice(0, -2));
        if (term.endsWith("ungen") && term.length > 8) out.add(term.slice(0, -5));
        if (term.endsWith("ung") && term.length > 6) out.add(term.slice(0, -3));
    });
...
/* Minimaler Handy-Menü-Fallback */
(function wtMinimalMobileMenuFallback() {
    if (window.__wtMinimalMobileMenuFallback) return;
    window.__wtMinimalMobileMenuFallback = true;
    ...
    function buildFallbackMenu() {
        ...
        overlay.innerHTML = `
            <div class="wt-mobile-menu-panel">
                <div class="wt-mobile-menu-head">
                    <strong>MENÜ</strong>
                    <button type="button" data-wt-menu-close>×</button>
                </div>
                <button type="button" data-wt-home>Startseite</button>
                <button type="button" data-wt-gallery>Tagesbilder</button>
                ${cats.map(cat => `<button type="button" data-wt-cat="${cat}">${cat}</button>`).join("")}
            </div>
        `;
    ...
    document.addEventListener("click", function(event) {
        if (event.target.closest("[data-wt-menu-close]")) {
            document.getElementById("wtMobileMenuFallback")?.remove();
            return;
        }

        if (event.target.closest("[data-wt-home]")) {
            document.getElementById("wtMobileMenuFallback")?.remove();
            if (typeof setView === "function") setView("home");
            return;
        }

        if (event.target.closest("[data-wt-gallery]")) {
            document.getElementById("wtMobileMenuFallback")?.remove();
            if (typeof setView === "function") setView("gallery");
            return;
        }
        ...
        if (cat) {
            document.getElementById("wtMobileMenuFallback")?.remove();
            if (cat.dataset.wtCat === "Spiele") {
                alert("Der Spiele-Bereich wird in dieser stabilen Ladefix-Version nicht automatisch geöffnet. Sudoku und Kreuzworträtsel bleiben auf der Startseite verfügbar.");
                return;
            }
            if (typeof executeSearchCategory === "function") executeSearchCategory(cat.dataset.wtCat);
            return;
        }
        ...
        const looksLikeMenu = onClick.includes("toggleMenu") || label.includes("menu") || label.includes("menü") || text === "☰";

        if (!looksLikeMenu) return;

        setTimeout(function() {
            const menuAlreadyOpen = document.querySelector(".fixed.inset-0.bg-black\\/60");
            if (!menuAlreadyOpen && window.innerWidth <= 900) {
                buildFallbackMenu();
            }
        }, 160);
    }, true);
