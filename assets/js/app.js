...
function wtSortArticlesWithBreakingFirst(list) {
    const source = Array.isArray(list) ? list.slice() : [];
    return source.sort((a, b) => {
        const ab = a && a.isEilmeldung ? 1 : 0;
        const bb = b && b.isEilmeldung ? 1 : 0;
...
function renderHome() {
    ...
    const now = new Date();
    const currentArticles = wtSortArticlesWithBreakingFirst(articles);
...
function renderSearchResults() {
    ...
    if (searchCategory) {
        results = wtSortArticlesWithBreakingFirst(currentArticles.filter(a => a.category === searchCategory));
        titleHtml = `Ressort: ${searchCategory}`;
    } else {
        const query = searchQuery;
        results = wtSortArticlesWithBreakingFirst(currentArticles.filter(a => wtArticleMatchesSearch(a, query)));
...
/* Tagesbilder lokaler Upload Fix */
(function wtGalleryUploadFix() {
    if (window.__wtGalleryUploadFix) return;
    window.__wtGalleryUploadFix = true;
    ...
    function fileToUrl(file) {
        return new Promise((resolve, reject) => {
            ...
            if (file.size > 3000000) return reject(new Error("Bild ist zu gross. Bitte unter 3 MB."));
    ...
    function imgs() {
        try { if (typeof communityImages !== "undefined" && Array.isArray(communityImages)) return communityImages; } catch (_) {}
        if (!Array.isArray(window.communityImages)) window.communityImages = [];
        return window.communityImages;
    }

    function save() {
        try { if (typeof saveState === "function") return saveState(); } catch (_) {}
        try { if (typeof scheduleRemoteSave === "function") scheduleRemoteSave(); } catch (_) {}
    }

    function install() {
        if (document.getElementById("wtGalleryUploadFix")) return;
        if (!/Tagesbilder|Community/i.test(document.body.innerText || "")) return;
        const target = document.querySelector("main") || document.body;
        const panel = document.createElement("section");
        panel.id = "wtGalleryUploadFix";
        panel.className = "wt-gallery-upload-panel";
        panel.innerHTML = '<h3>Tagesbild hochladen</h3><input id="wtGalleryUploadFile" type="file" accept="image/*"><input id="wtGalleryUploadCaption" type="text" placeholder="Bildbeschreibung optional"><button type="button">Bild hochladen</button><p></p>';
        target.prepend(panel);
        panel.querySelector("button").addEventListener("click", async () => {
            const status = panel.querySelector("p");
            try {
                const url = await fileToUrl(panel.querySelector("#wtGalleryUploadFile").files[0]);
                imgs().unshift({ id: Date.now(), url, caption: panel.querySelector("#wtGalleryUploadCaption").value || "", uploader: (typeof currentUser !== "undefined" && currentUser) ? currentUser : "Gast", timestamp: new Date().toISOString(), likes: [], isDeleted: false });
                await save();
                if (status) status.textContent = "Bild hochgeladen.";
