...
let supportChatAiToggleBusy = false;
...
function getSupportCategoriesForWorker() {
    try {
        if (typeof window.wtGetPublicSupportCategories === "function") {
            return window.wtGetPublicSupportCategories();
        }
    } catch (_) {}

    try {
        if (Array.isArray(categories)) {
            return Array.from(new Set(categories.map(cat => String(cat || "").trim()).filter(Boolean)));
        }
    } catch (_) {}
...
function getSupportPageContextForWorker() {
    try {
        if (typeof window.wtGetPublicSupportContext === "function") {
            return window.wtGetPublicSupportContext();
        }
    } catch (_) {}

    const cats = getSupportCategoriesForWorker();
    return [
        cats.length ? `Aktuelle Ressorts/Kategorien: ${cats.join(", ")}.` : "Aktuelle Ressorts/Kategorien: keine übermittelt.",
        "Professioneller Header: schwarze Top-Leiste, weißer Kopfbereich mit Logo links, Menü-Icon, Such-Icon, großem Titel Winterthur Times, Tagline und Datum/Wetter rechts.",
        "Favicon/Browser-Tab: Das Website-Logo ist als assets/logo.png in index.html eingebunden.",
        "Startseite: Hauptartikel, Artikelkarten, rechte Widget-Spalte mit Meistgelesen, Tägliches Rätsel, Tagesbilder, Sudoku und Kreuzworträtsel.",
        "Support-Chat: runder blauer Button unten rechts; im Fenster Eingabefeld und Senden-Button."
    ].join("\n");
}

function getSupportLoggedInState() {
    try {
    ...
    container.innerHTML = `
        ...
        <div id="support-chat-widget" class="fixed bottom-0 right-0 w-full h-[100dvh] sm:bottom-24 sm:right-6 sm:w-[420px] sm:h-[30rem] bg-white sm:border sm:border-gray-200 shadow-2xl sm:rounded-2xl z-[100] hidden flex-col overflow-hidden font-sans transition-all duration-300 transform origin-bottom-right">
            ...
            <div class="bg-white border-t border-gray-100 flex flex-col shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.05)] pb-safe">
                <div class="px-3 sm:px-4 pt-3 flex items-center justify-between text-xs text-gray-500">
                    <span>KI-Antworten für diesen Chat</span>
                    <button id="supportChatInlineAiToggleButton" type="button" class="px-3 py-1 rounded-full bg-green-100 text-green-700 font-bold border border-green-200 hover:bg-green-200 transition-colors">KI: AN</button>
                </div>
...
function applySupportAIState(enabled) {
    ...
    const inlineToggleBtn = document.getElementById('supportChatInlineAiToggleButton');
    ...
    if (aiEnabled) {
        ...
        if (inlineToggleBtn) {
            inlineToggleBtn.innerText = "KI: AN";
            inlineToggleBtn.className = "px-3 py-1 rounded-full bg-green-100 text-green-700 font-bold border border-green-200 hover:bg-green-200 transition-colors";
        }
    } else {
        ...
        if (inlineToggleBtn) {
            inlineToggleBtn.innerText = "KI: AUS";
            inlineToggleBtn.className = "px-3 py-1 rounded-full bg-gray-100 text-gray-700 font-bold border border-gray-300 hover:bg-gray-200 transition-colors";
        }
    }
...
async function toggleSupportAI() {
    if (supportChatAiToggleBusy) return;

    const previousState = aiEnabled;
    const nextState = !aiEnabled;
    supportChatAiToggleBusy = true;

    // Sofort anzeigen, damit sich der Button direkt anfühlt.
    applySupportAIState(nextState);

    try {
        const res = await fetch(`${WORKER_BASE}/api/chat/ai`, {
            method: "POST",
        ...
        console.error("Fehler beim Speichern des KI-Status:", err);
        applySupportAIState(previousState);

        chatMessages.push({
            role: "assistant",
            content: "Der KI-Status konnte gerade nicht gespeichert werden. Bitte versuche es erneut."
        });
        updateSupportChatUI();
    } finally {
        supportChatAiToggleBusy = false;
    }
}
    ...
    try {
        const res = await fetch(`${WORKER_BASE}/api/chat`, {
            ...
            body: JSON.stringify({
                userId: getSupportChatUserId(),
                message: text,
                loggedIn: getSupportLoggedInState(),
                popular: [],
                categories: getSupportCategoriesForWorker(),
                pageContext: getSupportPageContextForWorker()
            })
        });
...
// Globale Funktionen für onclick-Handler und dynamische Buttons
window.toggleSupportChat = toggleSupportChat;
window.toggleSupportAI = toggleSupportAI;
