...
// Supportbot-Seitenkontext: kann vom Chat-Frontend an den Worker gesendet werden.
window.getWinterthurTimesSupportPageContext = function () {
    const view = typeof currentView !== "undefined" ? currentView : "";
    const title = document.title || "Winterthur Times";
    const visibleHeadings = Array.from(document.querySelectorAll("h1,h2,h3,button,a"))
        .map(el => (el.textContent || "").trim())
        .filter(Boolean)
        .slice(0, 40)
        .join(" | ");

    return `Seite: ${location.pathname}${location.hash || ""}; View: ${view}; Titel: ${title}; Sichtbare Elemente: ${visibleHeadings}`;
};
