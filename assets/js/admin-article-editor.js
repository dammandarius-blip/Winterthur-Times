...
(function initAdvancedArticleEditor() {
    ...
    function isLoggedInOrAdminPanelVisible() {
        ...
        return bodyText.includes("admin") || bodyText.includes("artikel") || bodyText.includes("logout") || bodyText.includes("abmelden");
    }
            ...
            <div class="wt-editor-card">
                <div class="wt-editor-head">
                    <div>
                        ...
                        <p>Hier kannst du komplette Artikel korrekt für Firebase speichern.</p>
                    </div>
    ...
    async function saveArticleToFirebase(article) {
        if (!Array.isArray(window.articles) && typeof articles !== "undefined" && Array.isArray(articles)) {
            window.articles = articles;
        }

        if (!Array.isArray(window.articles)) {
            throw new Error("Die globale Artikelliste wurde nicht gefunden.");
        }

        window.articles.unshift(article);
        ...
        if (typeof saveData === "function") {
            ...
            await db.collection("data").doc("articles").set({ articles: window.articles }, { merge: true });
        } else {
