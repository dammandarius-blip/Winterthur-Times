...
(function initAdvancedArticleEditor() {
    ...
    function refreshAdminArticleListSoon() {
        try {
            if (typeof adminTab !== "undefined") adminTab = "articles";
            if (typeof window.adminTab !== "undefined") window.adminTab = "articles";
        } catch (_) {}

        if (typeof renderApp === "function") {
            setTimeout(() => {
                try { renderApp(); } catch (err) { console.error("Admin-Artikelliste konnte nicht aktualisiert werden:", err); }
            }, 120);
        }
    }
