...
/* =========================================================
   Stabiler individueller Artikel-Entwurf ohne 1GB-Speicher
   ========================================================= */
(function wtStableIndividualArticleDraft() {
    if (window.__wtStableIndividualArticleDraft) return;
    window.__wtStableIndividualArticleDraft = true;

    const STORAGE_PREFIX = "wt_article_draft_individual_v2:";
    const MAX_DRAFTS = 15;
    ...
    function getUserKey() {
        ...
        try {
            const raw = localStorage.getItem("wtCurrentUser") || localStorage.getItem("currentUser") || "";
            if (raw) return raw;
        } catch (_) {}
    ...
    function storageKey(id) {
        return STORAGE_PREFIX + getUserKey() + ":" + id;
    }

    function indexKey() {
        return STORAGE_PREFIX + getUserKey() + ":__index__";
    }

    function getEditor() {
        return document.getElementById("wt-advanced-article-editor");
    }

    function getModeText() {
        const editor = getEditor();
        return (editor?.innerText || "").includes("Bearbeiten") ? "edit" : "new";
    }
    ...
    function getDraftId() {
        ...
        const articleId = document.getElementById("wtArticleId")?.value || "";
        currentDraftId = mode + ":" + (articleId || "active") + ":" + getUserKey();

        // If title already exists and no articleId, keep same active draft; do not create new on every title change.
    ...
    function writeIndex(index) {
        try {
            localStorage.setItem(indexKey(), JSON.stringify(index.slice(0, MAX_DRAFTS)));
        } catch (_) {}
    }

    function pruneIndex() {
        const index = readIndex().sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0));
        index.slice(MAX_DRAFTS).forEach(item => {
            try { localStorage.removeItem(storageKey(item.id)); } catch (_) {}
        });
    ...
    function collectDraft() {
        ...
        const title = fields.wtArticleTitle || fields.title || "";
        const content = fields.wtArticleContent || fields.content || "";
        ...
        return {
            ...
            hasContent: !!(String(title).trim() || String(content).trim())
        };
    }

    function applyDraft(draft) {
        const editor = getEditor();
        if (!editor || !draft || !draft.fields) return;
        ...
        editor.querySelectorAll("input, textarea, select").forEach((el, index) => {
            ...
            if (el.type === "checkbox") el.checked = !!draft.fields[key];
    ...
    function setStatus(text, warning = false) {
        const el = document.getElementById("wtStableDraftStatus");
        if (!el) return;
        el.textContent = text;
    ...
    function saveDraftNow() {
        ...
        try {
            localStorage.setItem(storageKey(draft.id), json);

            const index = readIndex().filter(x => x.id !== draft.id);
            index.unshift({
                id: draft.id,
                title: draft.title || "Unbenannter Artikel",
                mode: draft.mode,
                updatedAt: draft.updatedAt,
                updatedIso: draft.updatedIso
            });
            writeIndex(index);
            pruneIndex();

            setStatus("Lokal gespeichert: " + new Date().toLocaleTimeString("de-CH", { hour: "2-digit", minute: "2-digit", second: "2-digit" }));
        } catch (err) {
            setStatus("Lokales Speichern fehlgeschlagen. Browser-Speicher voll?", true);
        }
    }
    ...
    function loadActiveDraft() {
        try {
            const raw = localStorage.getItem(storageKey(getDraftId()));
            if (!raw) {
                setStatus("Kein Entwurf für diesen Editor gefunden.");
                return;
            }
            applyDraft(JSON.parse(raw));
            setStatus("Entwurf geladen.");
        } catch (err) {
            setStatus("Entwurf konnte nicht geladen werden.", true);
        }
    }

    function clearActiveDraft() {
        try {
            localStorage.removeItem(storageKey(getDraftId()));
            writeIndex(readIndex().filter(x => x.id !== getDraftId()));
            lastSaved = "";
            editorWasDirty = false;
            setStatus("Aktueller Entwurf gelöscht.");
        } catch (_) {}
    }
    ...
    function loadDraftById(id) {
        try {
            ...
            currentDraftId = id;
            applyDraft(JSON.parse(raw));
            setStatus("Ausgewählter Entwurf geladen.");
        } catch (_) {
            setStatus("Entwurf konnte nicht geladen werden.", true);
        }
    }
    ...
    function updateDraftDropdown() {
        const select = document.getElementById("wtDraftSelect");
        if (!select) return;

        const current = select.value;
        const drafts = listDrafts();
        select.innerHTML = `<option value="">Gespeicherten Entwurf auswählen...</option>` + drafts.map(item => {
            const date = item.updatedAt ? new Date(item.updatedAt).toLocaleString("de-CH") : "";
            const label = `${item.title || "Unbenannter Artikel"} · ${date}`;
            return `<option value="${String(item.id).replace(/"/g, "&quot;")}">${label}</option>`;
        }).join("");
    ...
    function installBox() {
        const editor = getEditor();
        if (!editor || document.getElementById("wtStableDraftBox")) return;

        // Hide previous autosave boxes from earlier versions, but keep their data untouched.
        document.querySelectorAll("#wtArticleIndexedDbAutosaveBox, #wtArticleDraftBox").forEach(el => {
            el.style.display = "none";
        });

        const box = document.createElement("div");
        box.id = "wtStableDraftBox";
        box.className = "wt-stable-draft-box";
        box.innerHTML = `
            <div class="wt-stable-draft-main">
                <strong>Individuelle lokale Zwischenspeicherung aktiv</strong>
                <span id="wtStableDraftStatus">Dieser Entwurf bleibt in diesem Browser erhalten, auch wenn andere die Website aktualisieren.</span>
            </div>
            <div class="wt-stable-draft-actions">
                <select id="wtDraftSelect"></select>
                <button type="button" id="wtLoadStableDraft">Entwurf laden</button>
                <button type="button" id="wtClearStableDraft">Aktuellen Entwurf löschen</button>
            </div>
        `;
        editor.prepend(box);

        box.querySelector("#wtLoadStableDraft")?.addEventListener("click", () => {
            const selected = document.getElementById("wtDraftSelect")?.value;
            if (selected) loadDraftById(selected);
            else loadActiveDraft();
        });

        box.querySelector("#wtClearStableDraft")?.addEventListener("click", clearActiveDraft);

        box.querySelector("#wtDraftSelect")?.addEventListener("change", event => {
            if (event.target.value) loadDraftById(event.target.value);
        });
    ...
    // Important: after external Firebase/admin re-render, restore visible editor fields from the local draft
    // so a draft is not wiped by someone else's update.
    function restoreAfterExternalRender() {
        const editor = getEditor();
        if (!editor) return;
        if (!editorWasDirty) return;
    ...
    const observer = new MutationObserver(() => {
        installBox();
        restoreAfterExternalRender();
    });
    observer.observe(document.documentElement, { childList: true, subtree: true });

    if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", installBox);
    else installBox();
...
/* Kleine Stabilitäts-Guards */
(function wtSmallStabilityGuards() {
    if (window.__wtSmallStabilityGuards) return;
    window.__wtSmallStabilityGuards = true;

    window.addEventListener("unhandledrejection", event => {
        console.warn("Winterthur Times: abgefangene Promise-Rejection", event.reason);
    });

    window.addEventListener("error", event => {
        console.warn("Winterthur Times: JS-Fehler", event.error || event.message);
    });
})();
