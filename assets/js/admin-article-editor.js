...
/* =========================================================
   Artikel-Editor: Lokale 1GB IndexedDB-Zwischenspeicherung
   ========================================================= */
(function wtArticleIndexedDbAutosave1GB() {
    if (window.__wtArticleIndexedDbAutosave1GB) return;
    window.__wtArticleIndexedDbAutosave1GB = true;

    const DB_NAME = "WinterthurTimesArticleDrafts";
    const DB_VERSION = 1;
    const STORE = "drafts";
    const ACTIVE_ID = "active-article-draft";
    const MAX_BYTES = 1024 * 1024 * 1024; // ca. 1 GB
    const WARN_BYTES = 900 * 1024 * 1024;
    ...
    function tx(storeMode, callback) {
    ...
    async function getAllDrafts() {
        ...
        return new Promise((resolve, reject) => {
            ...
            req.onerror = () => {
                db.close();
                reject(req.error || new Error("Entwürfe konnten nicht gelesen werden."));
            };
        });
    }
    ...
    async function getDraft(id = ACTIVE_ID) {
        const db = await openDb();
        return new Promise((resolve, reject) => {
            const transaction = db.transaction(STORE, "readonly");
            const store = transaction.objectStore(STORE);
            ...
            req.onerror = () => {
                db.close();
                reject(req.error || new Error("Entwurf konnte nicht gelesen werden."));
            };
        });
    }
    ...
    async function totalDraftBytes() {
        const drafts = await getAllDrafts();
        return drafts.reduce((sum, draft) => sum + (draft.sizeBytes || byteSize(draft)), 0);
    }

    async function pruneOldDrafts() {
        let drafts = await getAllDrafts();
        let total = drafts.reduce((sum, draft) => sum + (draft.sizeBytes || byteSize(draft)), 0);
        ...
        for (const draft of drafts) {
            ...
            total -= draft.sizeBytes || byteSize(draft);
        }

        // Wenn sogar der aktive Entwurf alleine über 1GB wäre, kann der Browser ihn sowieso kaum speichern.
        // Dann lassen wir ihn als aktuellen Entwurf stehen, löschen aber alte Backups.
        return total;
    }
    ...
    function collectDraftPayload() {
        ...
        return {
            fields,
            url: location.href,
            title: fields.wtArticleTitle || fields.title || "",
    ...
    function applyDraftPayload(payload) {
        const editor = getEditor();
        if (!editor || !payload || !payload.fields) return;
        ...
        editor.querySelectorAll("input, textarea, select").forEach((el, index) => {
            ...
            if (el.type === "checkbox") {
                el.checked = !!payload.fields[key];
            } else {
                el.value = payload.fields[key] || "";
            }
    ...
    function setStatus(message, warning = false) {
        let status = document.getElementById("wtArticleIndexedDbAutosaveStatus");
        if (!status) return;
    ...
    async function saveDraftNow() {
        ...
        try {
            ...
            // Zusätzlich Snapshot mit Zeitstempel, damit alte Entwürfe bei Bedarf wiederherstellbar sind.
            const snapshot = {
                id: "draft-" + Date.now(),
                payload,
                updatedAt: Date.now(),
            ...
            const total = await pruneOldDrafts();
            const mb = Math.round(total / 1024 / 1024);
            setStatus(`Lokal gespeichert: ${new Date().toLocaleTimeString("de-CH")} · Speicher: ca. ${mb} MB / 1024 MB`, total > WARN_BYTES);
        } catch (err) {
            setStatus("Autospeichern fehlgeschlagen: " + (err.message || err), true);
        }
    }
    ...
    async function restoreActiveDraft() {
        try {
            const draft = await getDraft(ACTIVE_ID);
            if (!draft || !draft.payload) {
                setStatus("Kein lokaler Entwurf gefunden.");
                return;
            }
            applyDraftPayload(draft.payload);
            setStatus("Lokaler Entwurf geladen.");
        } catch (err) {
            setStatus("Entwurf konnte nicht geladen werden: " + (err.message || err), true);
        }
    }
    ...
    async function clearDrafts() {
        try {
            ...
            lastSavedJson = "";
            setStatus("Alle lokalen Artikel-Entwürfe gelöscht.");
        } catch (err) {
            setStatus("Entwürfe konnten nicht gelöscht werden: " + (err.message || err), true);
        }
    }

    async function showStorageInfo() {
        try {
            const total = await totalDraftBytes();
            const mb = Math.round(total / 1024 / 1024);
            setStatus(`Lokaler Entwurfsspeicher: ca. ${mb} MB / 1024 MB`, total > WARN_BYTES);
        } catch (_) {}
    }

    function installUi() {
        const editor = getEditor();
        if (!editor || document.getElementById("wtArticleIndexedDbAutosaveBox")) return;

        const oldBox = document.getElementById("wtArticleDraftBox");
        if (oldBox) oldBox.style.display = "none";

        const box = document.createElement("div");
        box.id = "wtArticleIndexedDbAutosaveBox";
        box.className = "wt-indexeddb-autosave-box";
        box.innerHTML = `
            <div>
                <strong>Lokale Artikel-Zwischenspeicherung aktiv</strong>
                <span id="wtArticleIndexedDbAutosaveStatus">Bereit. Maximal ca. 1 GB, danach werden alte Entwürfe überschrieben.</span>
            </div>
            <div class="wt-indexeddb-autosave-actions">
                <button type="button" id="wtIndexedDbLoadDraft">Entwurf laden</button>
                <button type="button" id="wtIndexedDbClearDrafts">Alle lokalen Entwürfe löschen</button>
            </div>
        `;

        editor.prepend(box);

        box.querySelector("#wtIndexedDbLoadDraft")?.addEventListener("click", restoreActiveDraft);
        box.querySelector("#wtIndexedDbClearDrafts")?.addEventListener("click", clearDrafts);

        editor.addEventListener("input", queueSave);
        editor.addEventListener("change", queueSave);

        setTimeout(restoreActiveDraft, 400);
        setTimeout(showStorageInfo, 900);
        setInterval(saveDraftNow, 5000);
    }
    ...
    window.addEventListener("beforeunload", () => {
        try {
            const payload = collectDraftPayload();
            if (!payload) return;
            // beforeunload kann async nicht zuverlässig abwarten, aber der normale 5s-Speicher läuft vorher.
            localStorage.setItem("wt_article_emergency_small_backup", JSON.stringify(payload).slice(0, 2000000));
        } catch (_) {}
    });
})();
