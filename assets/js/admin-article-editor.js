...
/* Artikel-Editor Auto-Speichern, Autor-Dropdown, lokales Artikelbild */
(function wtEditorAutosaveAuthorUpload() {
    if (window.__wtEditorAutosaveAuthorUpload) return;
    window.__wtEditorAutosaveAuthorUpload = true;

    const DRAFT_KEY = "wt_article_editor_draft_v5";

    function roleOf(u) {
        if (!u) return "user";
        return u.role || (u.isSuperAdmin ? "admin" : (u.isAdmin ? "admin" : "user"));
    }
    ...
    function authorChoices() {
        ...
        try {
            if (typeof authors !== "undefined" && Array.isArray(authors)) {
                authors.forEach(a => { if (a && a.name) out.push({ name: a.name, type: "Autor" }); });
            }
        } catch (_) {}
        try {
            if (typeof registeredUsers !== "undefined" && Array.isArray(registeredUsers)) {
                registeredUsers.forEach(u => {
                    const r = roleOf(u);
                    if (["admin", "superadmin", "author", "redaktor"].includes(String(r).toLowerCase())) {
                        const name = u.displayName || u.username || u.name || u.email;
                        if (name) out.push({ name, type: r.includes("admin") ? "Admin" : "Autor" });
                    }
                });
            }
        } catch (_) {}
        ["Redaktion", "Admin"].forEach(name => out.push({ name, type: "Standard" }));
        const seen = new Set();
        return out.filter(x => {
    ...
    function collectDraft() {
        const draft = {};
        document.querySelectorAll("#wt-advanced-article-editor input, #wt-advanced-article-editor textarea, #wt-advanced-article-editor select").forEach(el => {
            if (!el.id && !el.className) return;
            const key = el.id || ("class:" + String(el.className).split(" ")[0] + ":" + Array.from(document.querySelectorAll("." + String(el.className).split(" ")[0])).indexOf(el));
    ...
    function saveDraft() {
        ...
        try {
            ...
            const status = document.getElementById("wtAutosaveStatus");
            if (status) status.textContent = "Automatisch gespeichert: " + new Date().toLocaleTimeString("de-CH");
        } catch (err) {
            console.warn("Autosave Fehler:", err);
        }
    }
    ...
    function restoreDraft() {
        try {
            ...
            console.warn("Draft Restore Fehler:", err);
        }
    }
    ...
    function installAutosave() {
        const editor = document.getElementById("wt-advanced-article-editor");
        if (!editor || document.getElementById("wtAutosaveBox")) return;

        const box = document.createElement("div");
        box.id = "wtAutosaveBox";
        box.className = "wt-autosave-box";
        box.innerHTML = '<strong>Entwurf-Autospeicher aktiv</strong><span id="wtAutosaveStatus">Bereit.</span><button type="button" id="wtDraftRestore" class="wt-editor-secondary">Entwurf laden</button><button type="button" id="wtDraftClear" class="wt-editor-danger">Entwurf löschen</button>';
        editor.prepend(box);
        box.querySelector("#wtDraftRestore").addEventListener("click", restoreDraft);
        box.querySelector("#wtDraftClear").addEventListener("click", clearDraft);
    ...
    function installAuthorSelect() {
        ...
        select.innerHTML = '<option value="">Autor/Admin auswählen...</option>' + authorChoices().map(x => '<option value="' + String(x.name).replace(/"/g, "&quot;") + '">' + x.name + ' (' + x.type + ')</option>').join("");
    ...
    function imageFileToUrl(file) {
        return new Promise((resolve, reject) => {
            if (!file) return reject(new Error("Keine Datei ausgewählt."));
            if (!file.type.startsWith("image/")) return reject(new Error("Bitte Bilddatei auswählen."));
            if (file.size > 2500000) return reject(new Error("Bild ist zu gross. Bitte unter 2.5 MB."));
    ...
    function installArticleImageUpload() {
        const input = document.getElementById("wtArticleImageUrl");
        if (!input || document.getElementById("wtArticleLocalImageUpload")) return;
        const panel = document.createElement("div");
        panel.id = "wtArticleLocalImageUpload";
        panel.className = "wt-local-upload-panel";
        panel.innerHTML = '<label>Lokales Artikelbild hochladen<input id="wtArticleLocalImageFile" type="file" accept="image/*"></label><button type="button" class="wt-editor-secondary">Bild übernehmen</button><p>Kleine Bilder werden direkt als Bildadresse gespeichert.</p>';
        input.insertAdjacentElement("afterend", panel);
        panel.querySelector("button").addEventListener("click", async () => {
            const status = document.getElementById("wtAutosaveStatus");
            try {
                const url = await imageFileToUrl(panel.querySelector("#wtArticleLocalImageFile").files[0]);
                input.value = url;
                saveDraft();
                if (status) status.textContent = "Artikelbild übernommen.";
            } catch (err) {
