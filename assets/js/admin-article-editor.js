...
(function initAdvancedArticleEditor() {
    ...
    function mountEditorWhenPossible() {
        ...
        if (!host && !isLoggedInOrAdminPanelVisible()) return false;

        const existing = document.getElementById(ADVANCED_EDITOR_ID);
        if (existing && existing.isConnected && (!host || host.contains(existing))) return true;
    ...
    window.wtMountAdvancedArticleEditor = function() {
        return mountEditorWhenPossible();
    };
    ...
    function buildArticleFromForm() {
        ...
        const seenInlinePositions = new Set();
        const mainImageKey = typeof getArticleImageUrlKey === "function"
            ? getArticleImageUrlKey(imageUrl)
            : imageUrl.trim().toLowerCase();
        if (mainImageKey) seenImageUrls.add(mainImageKey);
        ...
        const inlineImages = Array.from(document.querySelectorAll(".wt-inline-image-row")).map(row => {
            ...
            if (!(img.positionAfterParagraph > 0) || !img.url) return false;
            img.positionAfterParagraph = Math.floor(img.positionAfterParagraph);
            if (seenInlinePositions.has(img.positionAfterParagraph)) return false;
            const key = typeof getArticleImageUrlKey === "function"
                ? getArticleImageUrlKey(img.url)
                : img.url.trim().toLowerCase();
            if (!key || seenImageUrls.has(key)) return false;
            seenImageUrls.add(key);
            seenInlinePositions.add(img.positionAfterParagraph);
            return true;
        });
    ...
    async function saveArticleToFirebase(article) {
        if (typeof window.wtEnsureArticleInAdminList === "function") {
            window.wtEnsureArticleInAdminList(article);
        }
        ...
        try {
            window.dispatchEvent(new CustomEvent("wtArticleSaved", { detail: { article } }));
        } catch (_) {}
...
Die Klimawoche zeigt, dass Klimaanpassung in Winterthur bereits jetzt ein wichtiges Thema ist.`;
    ...
    function persistArticlesToFirebase() {
        if (typeof window.wtPersistArticlesSafely === "function") {
            return window.wtPersistArticlesSafely(articles);
        }
        if (typeof firebaseDb !== "undefined" && firebaseDb && typeof firebase !== "undefined") {
    ...
    async function updateExistingArticleFromEditor(articleFromForm) {
        ...
        if (typeof window.wtEnsureArticleInAdminList === "function") {
            window.wtEnsureArticleInAdminList(updated);
        }
        try {
            window.dispatchEvent(new CustomEvent("wtArticleSaved", { detail: { article: updated } }));
        } catch (_) {}
    ...
    function bindAdvancedEditor(wrapper) {
        ...
        wrapper.querySelector("#wtSaveArticle")?.addEventListener("click", async () => {
            try {
                ...
                status.textContent = wasEditing ? "Artikel wurde erfolgreich aktualisiert." : "Artikel wurde erfolgreich in Firebase gespeichert. Er erscheint jetzt unten in der Admin-Liste.";
                status.className = "wt-editor-status ok";
                if (typeof window.wtRefreshAdminArticleListNow === "function") {
                    window.wtRefreshAdminArticleListNow(savedArticle || article);
                }
            } catch (err) {
...
/* Editor: zuletzt gespeicherter Artikel darf gelöschte Artikel nicht zurückbringen */
(function wtEditorDeletedArticleGuard() {
    if (window.__wtEditorDeletedArticleGuard) return;
    window.__wtEditorDeletedArticleGuard = true;

    setInterval(() => {
        try {
            if (typeof window.wtArticleIsDeletedHard !== "function") return;
            const raw = localStorage.getItem("wt_articles_admin_last_save");
            if (!raw) return;
            const data = JSON.parse(raw);
            if (data && data.article && window.wtArticleIsDeletedHard(data.article)) {
